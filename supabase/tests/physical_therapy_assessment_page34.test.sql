...
select is(
  ...
  3::bigint,
  'page 34 publishes exactly three governed permissions'
);
...
select ok(
  exists (
    ...
    where role.role_key = 'professional'
      and permission.permission_key = 'physical_therapy_assessments.manage'
  ) and not exists (
    select 1 from public.role_permissions role_permission
    join public.roles role on role.id = role_permission.role_id
    join public.permissions permission
...
select ok(
  (select bool_and(relrowsecurity and relforcerowsecurity)
   from pg_class where oid in (
    'public.physical_therapy_assessment_versions'::regclass,
    'private.physical_therapy_assessment_operations'::regclass
  )),
...
select ok(
  has_function_privilege('authenticated',
    'public.create_physical_therapy_assessment_draft(uuid,uuid,uuid,date,date,text,jsonb,text,text,text,text,text,uuid)',
    'execute')
  and has_function_privilege('authenticated',
    'public.revise_physical_therapy_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,uuid)',
    'execute')
  and has_function_privilege('authenticated',
    'public.sign_physical_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)',
    'execute')
  and has_function_privilege('authenticated',
    'public.correct_physical_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)',
    'execute')
  and has_function_privilege('authenticated',
    'public.physical_therapy_assessment_snapshot(uuid,uuid,uuid,uuid,text,text)',
    'execute')
...
select ok(
  not (select prosecdef from pg_proc where oid =
    'public.create_physical_therapy_assessment_draft(uuid,uuid,uuid,date,date,text,jsonb,text,text,text,text,text,uuid)'::regprocedure)
  and not (select prosecdef from pg_proc where oid =
    'public.physical_therapy_assessment_snapshot(uuid,uuid,uuid,uuid,text,text)'::regprocedure),
...
select ok(
  (select prosecdef and coalesce(proconfig, '{}'::text[]) @>
   ...
   from pg_proc where oid =
    'private.mutate_physical_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)'::regprocedure)
  and (select prosecdef and coalesce(proconfig, '{}'::text[]) @>
      array['search_path=""']
   from pg_proc where oid =
    'private.physical_therapy_assessment_snapshot_response(uuid,uuid,uuid,uuid,text,text)'::regprocedure)
  and not has_function_privilege('public',
    'private.mutate_physical_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)',
    'execute'),
...
select ok(
  position('pg_advisory_xact_lock' in pg_get_functiondef(
    'private.mutate_physical_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)'::regprocedure)) > 0
  and position('authority expired' in pg_get_functiondef(
    'private.mutate_physical_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)'::regprocedure)) > 0
  and position('v_after is distinct from v_bundle' in pg_get_functiondef(
    'private.physical_therapy_assessment_snapshot_response(uuid,uuid,uuid,uuid,text,text)'::regprocedure)) > 0,
  'writes serialize and reads and writes recheck authority'
);

select is(
  (select count(*) from pg_trigger where not tgisinternal and tgname in (
    'physical_therapy_assessment_versions_append_only',
    'physical_therapy_assessment_operations_append_only'
  )), 2::bigint,
...
select is(
  (select count(*) from pg_trigger where not tgisinternal and tgname in (
    'physical_therapy_assessment_versions_audit_row_change',
    'physical_therapy_assessment_operations_audit_row_change'
  )), 2::bigint,
...
select is(
  (select count(*) from information_schema.columns
   where table_schema = 'public'
     and table_name = 'physical_therapy_assessment_versions'
     and column_name in (
       'score', 'total_score', 'clinical_score', 'diagnosis',
       'formula', 'risk_level', 'automatic_due_on'
     )), 0::bigint,
  'page 34 stores no invented score formula diagnosis or automatic due date'
);

select ok(private.physical_therapy_measurements_are_valid(
  '[
...
select ok(not private.physical_therapy_measurements_are_valid(
...
insert into auth.users (id, aud, role, email, created_at, updated_at) values
  ...
  ('33000000-0000-4000-8000-000000001002', 'authenticated', 'authenticated', 'therapist34@example.invalid', now(), now()),
  ('33000000-0000-4000-8000-000000001003', 'authenticated', 'authenticated', 'othertherapist34@example.invalid', now(), now()),
  ('33000000-0000-4000-8000-000000001004', 'authenticated', 'authenticated', 'othermanager33@example.invalid', now(), now());

insert into public.organizations (id, code, name) values
  ('33100000-0000-4000-8000-000000000001', 'pt_page34_a', '物理治療測試機構 A'),
  ('33100000-0000-4000-8000-000000000002', 'pt_page34_b', '物理治療測試機構 B');
...
insert into public.profiles (id, display_name, kind) values
  ...
  ('33000000-0000-4000-8000-000000001002', '合成物理治療師甲', 'professional'),
  ('33000000-0000-4000-8000-000000001003', '合成物理治療師乙', 'professional'),
...
insert into public.clients (
  ...
  ('33400000-0000-4000-8000-000000000001', '33100000-0000-4000-8000-000000000001', '33200000-0000-4000-8000-000000000001', 'PT-1', '合成個案甲', 'active', current_date - 90, null),
  ('33400000-0000-4000-8000-000000000002', '33100000-0000-4000-8000-000000000001', '33200000-0000-4000-8000-000000000001', 'PT-2', '合成個案乙', 'suspended', current_date - 60, null),
  ('33400000-0000-4000-8000-000000000003', '33100000-0000-4000-8000-000000000001', '33200000-0000-4000-8000-000000000002', 'PT-3', '合成他分支個案', 'active', current_date - 90, null),
  ('33400000-0000-4000-8000-000000000004', '33100000-0000-4000-8000-000000000002', '33200000-0000-4000-8000-000000000003', 'PT-4', '合成他機構個案', 'active', current_date - 90, null);

insert into public.client_assignments (
  id, organization_id, branch_id, client_id, assignee_user_id,
  assignment_kind
) values
  ('33500000-0000-4000-8000-000000000001', '33100000-0000-4000-8000-000000000001', '33200000-0000-4000-8000-000000000001', '33400000-0000-4000-8000-000000000001', '33000000-0000-4000-8000-000000001002', 'physical-therapy'),
  ('33500000-0000-4000-8000-000000000002', '33100000-0000-4000-8000-000000000001', '33200000-0000-4000-8000-000000000001', '33400000-0000-4000-8000-000000000001', '33000000-0000-4000-8000-000000001003', 'physical-therapy-review');
...
insert into private.reauth_challenges (
  ...
  '33000000-0000-4000-8000-000000001003',
  '33610000-0000-4000-8000-000000000098', repeat('4', 64),
  '33620000-0000-4000-8000-000000000002',
  clock_timestamp() - interval '2 minutes',
  clock_timestamp() - interval '2 minutes',
  clock_timestamp() + interval '6 minutes',
  clock_timestamp() - interval '30 seconds',
...
insert into private.reauth_events (
  ...
  '33000000-0000-4000-8000-000000001003',
  '33610000-0000-4000-8000-000000000098',
  '33600000-0000-4000-8000-000000000002', 'aal2', 'totp',
  (select factor_verified_at from private.reauth_challenges
   where id = '33600000-0000-4000-8000-000000000002')
);
...
select throws_ok($$select * from public.physical_therapy_assessment_snapshot(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001')$$,
  '42501', 'physical_therapy assessment snapshot is not permitted',
  'reads require AAL2'
);
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  ...
  'manual-physical-therapy-v1',
  '33900000-0000-4000-8000-000000000099')$$,
  '42501',
  'current same-session recent AAL2 evidence is required for physical_therapy writing',
  'draft creation requires recent same-session AAL2 before content is committed'
);
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  ...
  '42501', 'physical_therapy assessment operation is not permitted',
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  ...
  '42501', 'physical_therapy assessment operation is not permitted',
  'non-professional manager cannot write an physical therapy assessment'
);
...
select * from public.create_physical_therapy_assessment_draft(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
...
select ok(
  (select due_basis = '人工排定：合成服務會議紀錄'
      ...
      and (select operation.reauth_challenge_id =
          '33600000-0000-4000-8000-000000000001'
        from private.physical_therapy_assessment_operations operation
        where operation.result_version_id =
          (select version_id from ot_create_result))
...
select ok(
  (select replayed
   ...
   from public.create_physical_therapy_assessment_draft(
    '33100000-0000-4000-8000-000000000001',
    '33200000-0000-4000-8000-000000000001',
    '33400000-0000-4000-8000-000000000001', current_date - 3,
    current_date + 7, '人工排定：合成服務會議紀錄',
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  ...
  'manual-physical-therapy-v1',
  '33900000-0000-4000-8000-000000000010')$$,
  '23505', 'physical_therapy assessment idempotency conflict',
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  ...
  (select recommendations from ot_payloads where payload_key = 'first'),
  (select follow_up_plan from ot_payloads where payload_key = 'first'),
  'official-physical-therapy-scale',
  '33900000-0000-4000-8000-000000000011')$$,
  '22023', 'manual physical_therapy assessment content is invalid',
...
select throws_ok($$select * from public.create_physical_therapy_assessment_draft(
  ...
  '22023', 'future physical_therapy assessment date is invalid',
  'future assessment date is rejected'
);

insert into ot_revise_result
select * from public.revise_physical_therapy_assessment_draft(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
...
select throws_ok($$select * from public.revise_physical_therapy_assessment_draft(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  '33400000-0000-4000-8000-000000000001',
  (select assessment_key from ot_create_result),
  ...
  '40001', 'physical_therapy assessment version is stale',
  'stale expected version is rejected'
);

select set_config('request.jwt.claims',
  '{"sub":"33000000-0000-4000-8000-000000001002","role":"authenticated","aal":"aal2","session_id":"33610000-0000-4000-8000-000000000099"}', true);
select throws_ok($$select * from public.sign_physical_therapy_assessment(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  '33400000-0000-4000-8000-000000000001',
...
select throws_ok($$select * from public.sign_physical_therapy_assessment(
  ...
  '42501', 'only the current qualified physical therapist may sign this draft',
...
select * from public.sign_physical_therapy_assessment(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  '33400000-0000-4000-8000-000000000001',
  (select assessment_key from ot_revise_result),
...
select throws_ok($$select * from public.correct_physical_therapy_assessment(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  '33400000-0000-4000-8000-000000000001',
  ...
  'manual-physical-therapy-v1', null,
...
select * from public.correct_physical_therapy_assessment(
  '33100000-0000-4000-8000-000000000001',
  '33200000-0000-4000-8000-000000000001',
  '33400000-0000-4000-8000-000000000001',
  (select assessment_key from ot_sign_result),
...
select throws_ok($$select * from public.revise_physical_therapy_assessment_draft(
  ...
  '23514', 'signed physical_therapy assessment requires a correction',
  'signed or corrected assessment cannot return to draft'
);

reset role;
select throws_ok($$update public.physical_therapy_assessment_versions
  set functional_observation = '直接覆寫'
  where id = (select version_id from ot_sign_result)$$,
  '55000', 'physical therapy assessment history is append-only',
  'direct update is rejected'
);
select throws_ok($$delete from public.physical_therapy_assessment_versions
  where id = (select version_id from ot_sign_result)$$,
  '55000', 'physical therapy assessment history is append-only',
...
select ok(
  (select matching_total = 2 and item_total = 2
   ...
   from public.physical_therapy_assessment_snapshot(
    '33100000-0000-4000-8000-000000000001',
    '33200000-0000-4000-8000-000000000001')),
...
select throws_ok($$select * from public.physical_therapy_assessment_snapshot(
  ...
  '42501', 'physical_therapy therapist filter is not permitted',
...
select ok(
  not exists (
    ...
    where event.table_name in (
      ...
      or event.metadata::text like '%33400000-0000-4000-8000-000000000001%'
      or event.metadata::text like '%33000000-0000-4000-8000-000000001004%'
    )
  ) and not exists (
    select 1 from public.audit_events event
    where event.table_name = 'physical_therapy_assessment_versions'
...
select is(
  (select count(*) from public.physical_therapy_assessment_versions
...
select is(
  (select count(*) from private.physical_therapy_assessment_operations
...
select ok(
  position('from matching' in pg_get_functiondef(
    'private.physical_therapy_assessment_snapshot_bundle(uuid,uuid,uuid,uuid,text,text,timestamptz)'::regprocedure)) > 0
...
select is(
  (select count(*) from public.audit_events
   where table_name = 'physical_therapy_assessment_versions'
     and action = 'select'),
  8::bigint,
