...
select throws_ok($$select public.read_store_attendance_month('a0500000-0000-4000-8000-000000000001','a0600000-0000-4000-8000-000000000001','2026-09')$$,'42501',null,'no-policy monthly aggregate denied');
reset role;
...
create function pg_temp.store_month(p_month text default '2026-09',p_org uuid default 'a0500000-0000-4000-8000-000000000001',p_branch uuid default 'a0600000-0000-4000-8000-000000000001')
returns jsonb language sql security invoker as $$ select public.read_store_attendance_month(p_org,p_branch,p_month); $$;
select ok((select not prosecdef and proconfig @> array['search_path=""'] from pg_proc where oid='public.read_store_attendance_month(uuid,uuid,text)'::regprocedure),'month entrypoint is invoker');
select ok((select prosecdef and proconfig @> array['search_path=""'] and pg_get_userbyid(proowner)='postgres' from pg_proc where oid='private.read_store_attendance_month(uuid,uuid,text)'::regprocedure),'month private function is owner pinned');
select ok(not has_function_privilege(r,f,'execute'),r||' denied '||f)
from unnest(array['anon','service_role']) r cross join unnest(array['public.read_store_attendance_month(uuid,uuid,text)','private.read_store_attendance_month(uuid,uuid,text)']) f;
set local role authenticated;
select is(pg_temp.store_month()->>'month','2026-09','month scoped');
select is(jsonb_array_length(pg_temp.store_month()->'days'),30,'all September dates including no records');
select is(pg_temp.store_month()->'days'->0,'{"date":"2026-09-01","present":2,"leave":1,"absent":1}'::jsonb,'first day matches current roots, not correction or cancelled rows');
select is(pg_temp.store_month()->'days'->1,'{"date":"2026-09-02","present":1,"leave":0,"absent":0}'::jsonb,'adjacent day separate');
select is(pg_temp.store_month()->'totals','{"present":3,"leave":1,"absent":1}'::jsonb,'daily rows sum to month person-days');
select is((pg_temp.store_month()->>'distinct_present_clients')::int,3,'distinct actual clients, independent from all active clients');
select is(jsonb_array_length(pg_temp.store_month('2024-02')->'days'),29,'leap February');
select is(jsonb_array_length(pg_temp.store_month('2026-02')->'days'),28,'ordinary February');
select is(jsonb_array_length(pg_temp.store_month('2026-08')->'days'),31,'long month');
select throws_ok($$select pg_temp.store_month('2026-13')$$,'22023',null,'bad month rejected');
select throws_ok($$select pg_temp.store_month(null)$$,'22023',null,'null month rejected');
select throws_ok($$select pg_temp.store_month('2026-09','a0500000-0000-4000-8000-000000000002','a0600000-0000-4000-8000-000000000003')$$,'42501',null,'foreign monthly totals denied');
set local timezone='America/Los_Angeles';
select is(pg_temp.store_month()->'totals','{"present":3,"leave":1,"absent":1}'::jsonb,'business month independent of database timezone');
set local timezone='Asia/Taipei';
reset role;
select ok((select bool_and(row_pk is null and changed_fields='{}'::text[] and metadata-'projection'-'month'='{}'::jsonb and action='select') from public.audit_events where metadata->>'projection'='store_attendance_month'),'month audit never includes names, values or counts');
...
select throws_ok($$select pg_temp.store_month('2026-09','a0500000-0000-4000-8000-000000000001','a0600000-0000-4000-8000-000000000002')$$,'42501',null,'monthly aggregate repeats branch permission check');
reset role;
...
select throws_ok($$select pg_temp.store_month()$$,'42501',null,'suspended membership cannot read monthly aggregate');
reset role;
