-- Page 11: SPMSQ candidate-rule drafts and reproducible trial previews.
--
...
insert into public.permissions (permission_key, description, risk_level) values
  ('assessments.read', 'Read assigned-client governed assessment drafts', 2),
  ('assessments.manage', 'Create and revise assigned-client governed assessment drafts', 3)
...
where role.is_system
  ...
  and permission.permission_key in ('assessments.read', 'assessments.manage')
on conflict (role_id, permission_id) do nothing;

create or replace function private.spmsq_candidate_rule_snapshot()
returns jsonb
language sql
immutable
security invoker
...
as $$
  select jsonb_build_object(
    'version_id', 'spmsq-pfeiffer-10-education-adjusted-v1',
    'instrument', 'spmsq',
    'rule_revision', 1,
    ...
    'answer_weights', jsonb_build_object('correct', 0, 'incorrect', 1),
    'missing_policy', 'no_preview_and_never_zero',
    'not_applicable_policy', 'no_preview_and_never_zero',
    'education_adjustment', jsonb_build_object(
    ...
    'cultural_adjustment', jsonb_build_object(
      'status', 'not_configured',
      'numeric_effect', 0,
      'policy', 'context_is_preserved_but_never_changes_trial_preview'
    ),
    'candidate_bands', jsonb_build_array(
      jsonb_build_object('key', 'reference_0_2_errors', 'min', 0, 'max', 2),
      jsonb_build_object('key', 'mild_3_4_errors', 'min', 3, 'max', 4),
      jsonb_build_object('key', 'moderate_5_7_errors', 'min', 5, 'max', 7),
      jsonb_build_object('key', 'high_8_10_errors', 'min', 8, 'max', 10)
    ),
    'disclaimer', 'Candidate trial preview only; not a diagnosis, official result, signature, or care decision.'
  );
$$;

create or replace function private.spmsq_answers_are_valid(p_answers jsonb)
returns boolean
language sql
immutable
security invoker
...
as $$
  select jsonb_typeof(p_answers) = 'object'
    and (select count(*) from jsonb_object_keys(p_answers)) = 10
    and not exists (
      select 1
      ...
      where jsonb_typeof(entry.value) <> 'object'
        ...
        or (
          ...
          and (
            ...
            or entry.value ->> 'value' not in ('correct', 'incorrect')
          )
        )
        or (
...
create or replace function private.spmsq_education_context_is_valid(
  p_context jsonb
)
returns boolean
language sql
...
as $$
  select jsonb_typeof(p_context) = 'object'
    and jsonb_typeof(p_context -> 'state') = 'string'
    and p_context ->> 'state' in ('answered', 'missing', 'not_applicable')
    and case p_context ->> 'state'
      when 'answered' then
        (select count(*) from jsonb_object_keys(p_context)) = 2
        and p_context ?& array['state', 'value']
        and jsonb_typeof(p_context -> 'value') = 'string'
        and p_context ->> 'value' in (
          'grade_school_or_less', 'middle_or_high_school',
          'beyond_high_school'
        )
      when 'missing' then
        (select count(*) from jsonb_object_keys(p_context)) = 1
        and p_context ? 'state'
      else
        (select count(*) from jsonb_object_keys(p_context)) = 2
        and p_context ?& array['state', 'reason']
        and jsonb_typeof(p_context -> 'reason') = 'string'
        and char_length(btrim(p_context ->> 'reason')) between 1 and 500
        and p_context ->> 'reason' = btrim(p_context ->> 'reason')
        and translate(p_context ->> 'reason', E'\n\r\t', '') !~ '[[:cntrl:]]'
    end;
$$;

create or replace function private.spmsq_cultural_context_is_valid(
  p_context jsonb
)
returns boolean
language sql
...
as $$
  select jsonb_typeof(p_context) = 'object'
    and jsonb_typeof(p_context -> 'state') = 'string'
    and p_context ->> 'state' in ('recorded', 'missing', 'not_applicable')
    and case p_context ->> 'state'
      when 'recorded' then
        (select count(*) from jsonb_object_keys(p_context)) = 2
        and p_context ?& array['state', 'note']
        and jsonb_typeof(p_context -> 'note') = 'string'
        and char_length(btrim(p_context ->> 'note')) between 1 and 2000
        and p_context ->> 'note' = btrim(p_context ->> 'note')
        and translate(p_context ->> 'note', E'\n\r\t', '') !~ '[[:cntrl:]]'
      when 'missing' then
        (select count(*) from jsonb_object_keys(p_context)) = 1
        and p_context ? 'state'
      else
...
create or replace function private.spmsq_trial_preview(
  p_answers jsonb,
  p_education_context jsonb
)
returns jsonb
language plpgsql
immutable
...
declare
  ...
  v_band text;
begin
  if not private.spmsq_answers_are_valid(p_answers)
     or not private.spmsq_education_context_is_valid(p_education_context) then
    return jsonb_build_object(
      'status', 'invalid', 'raw_errors', null,
      'adjusted_errors', null, 'band_key', null
    );
  end if;
  ...
  ) or p_education_context ->> 'state' <> 'answered' then
    return jsonb_build_object(
      'status', 'incomplete', 'raw_errors', null,
      'adjusted_errors', null, 'band_key', null
    );
  end if;
  ...
  from jsonb_each(p_answers) entry
  where entry.value ->> 'value' = 'incorrect';
  v_adjusted := v_raw + case p_education_context ->> 'value'
    when 'grade_school_or_less' then -1
    when 'beyond_high_school' then 1
    else 0
  end;
  v_adjusted := greatest(0, least(10, v_adjusted));
  v_band := case
    when v_adjusted between 0 and 2 then 'reference_0_2_errors'
    when v_adjusted between 3 and 4 then 'mild_3_4_errors'
    when v_adjusted between 5 and 7 then 'moderate_5_7_errors'
    else 'high_8_10_errors'
  end;
  return jsonb_build_object(
    'status', 'candidate_complete', 'raw_errors', v_raw,
    'adjusted_errors', v_adjusted, 'band_key', v_band
  );
end;
$$;

create table public.spmsq_assessment_versions (
  ...
  constraint spmsq_assessment_service_status_check check (
    service_status_at_assessment in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint spmsq_assessment_answers_check check (
    private.spmsq_answers_are_valid(answers)
  ),
  constraint spmsq_assessment_education_check check (
    private.spmsq_education_context_is_valid(education_context)
  ),
  constraint spmsq_assessment_cultural_check check (
    private.spmsq_cultural_context_is_valid(cultural_context)
  ),
  constraint spmsq_assessment_rule_check check (
    rule_version_id = 'spmsq-pfeiffer-10-education-adjusted-v1'
    and governance_status = 'candidate_unactivated'
    and rule_snapshot = private.spmsq_candidate_rule_snapshot()
    and rule_snapshot_hash = encode(
      sha256(convert_to(rule_snapshot::text, 'UTF8')), 'hex'
    )
  ),
  constraint spmsq_assessment_preview_check check (
    preview_status = private.spmsq_trial_preview(
      answers, education_context
    ) ->> 'status'
    and preview_raw_errors is not distinct from (
      private.spmsq_trial_preview(answers, education_context)
        ->> 'raw_errors'
    )::integer
    and preview_adjusted_errors is not distinct from (
      private.spmsq_trial_preview(answers, education_context)
        ->> 'adjusted_errors'
    )::integer
    and preview_band_key is not distinct from
      private.spmsq_trial_preview(answers, education_context) ->> 'band_key'
    and preview_status in ('candidate_complete', 'incomplete')
  ),
  constraint spmsq_assessment_hash_check check (
    content_hash ~ '^[a-f0-9]{64}$'
  )
);

comment on table public.spmsq_assessment_versions is
  'Immutable Page 11 candidate-rule drafts. Unactivated previews cannot be signed or used as official SPMSQ results.';
comment on column public.spmsq_assessment_versions.answers is
  'All ten answer slots with explicit answered, missing, or not-applicable state; values are never written to audit metadata.';
comment on column public.spmsq_assessment_versions.cultural_context is
  'Recorded context only. The candidate snapshot explicitly gives cultural context no numeric effect.';

create table private.spmsq_assessment_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  reauth_challenge_id uuid not null
    references private.reauth_challenges(id) on delete restrict,
  created_at timestamptz not null default clock_timestamp(),
  constraint spmsq_assessment_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint spmsq_assessment_operations_result_scope_fkey
    foreign key (
  ...
  constraint spmsq_assessment_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft')
  ),
  constraint spmsq_assessment_operations_request_hash_check check (
    request_hash ~ '^[a-f0-9]{64}$'
  ),
  constraint spmsq_assessment_operations_result_check check (
    result_version > 0 and result_state = 'draft_preview'
  )
);

create index spmsq_assessments_scope_latest_idx
  on public.spmsq_assessment_versions (
    organization_id, branch_id, client_id, assessed_on desc,
    created_at desc, assessment_key, version desc
  );
create index spmsq_assessments_chain_idx
  on public.spmsq_assessment_versions (
    organization_id, branch_id, assessment_key, version desc
  );
create index spmsq_assessments_previous_idx
  on public.spmsq_assessment_versions (previous_version_id)
  where previous_version_id is not null;
create index spmsq_assessments_author_idx
  on public.spmsq_assessment_versions (author_user_id);
create index spmsq_assessments_reauth_idx
  on public.spmsq_assessment_versions (write_reauth_challenge_id);
create index spmsq_operations_result_idx
  on private.spmsq_assessment_operations (
    organization_id, branch_id, client_id, result_assessment_key,
    result_version_id
  );
create index spmsq_operations_reauth_idx
  on private.spmsq_assessment_operations (reauth_challenge_id);

alter table public.spmsq_assessment_versions enable row level security;
alter table public.spmsq_assessment_versions force row level security;
alter table private.spmsq_assessment_operations enable row level security;
alter table private.spmsq_assessment_operations force row level security;

revoke all on table public.spmsq_assessment_versions
  from public, anon, authenticated, service_role;
revoke all on table private.spmsq_assessment_operations
  from public, anon, authenticated, service_role;

create or replace function private.spmsq_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'SPMSQ candidate assessment history is append-only';
end;
$$;
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'current same-session recent AAL2 evidence is required for SPMSQ draft writes';
  end if;
  begin
...
create or replace function private.mutate_spmsq_assessment_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_request_hash text;
  v_operation private.spmsq_assessment_operations%rowtype;
  v_previous public.spmsq_assessment_versions%rowtype;
  v_result public.spmsq_assessment_versions%rowtype;
  v_assessment_key uuid;
  v_version integer;
  v_author_display_name text;
  v_service_status text;
  v_rule_snapshot jsonb := private.spmsq_candidate_rule_snapshot();
  v_rule_hash text;
  v_preview jsonb;
  v_challenge_id uuid;
  v_content_hash text;
begin
  if p_action not in ('create_draft', 'revise_draft')
     ...
     or not private.spmsq_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'SPMSQ candidate assessment operation is not permitted';
  end if;

  v_challenge_id := private.require_spmsq_reauth_evidence(v_actor, v_now);
  ...
  if p_action = 'create_draft' then
    if p_assessment_key is not null or p_previous_version_id is not null
      ...
      raise exception using errcode = '22023',
        message = 'new SPMSQ assessment chain input is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'SPMSQ assessment chain input is invalid';
  end if;
  ...
  if p_assessed_on is null
     ...
     or p_rule_version_id <> 'spmsq-pfeiffer-10-education-adjusted-v1'
     or not private.spmsq_answers_are_valid(p_answers)
     or not private.spmsq_education_context_is_valid(p_education_context)
     or not private.spmsq_cultural_context_is_valid(p_cultural_context) then
    raise exception using errcode = '22023',
      message = 'SPMSQ candidate assessment payload is invalid';
  end if;

  v_preview := private.spmsq_trial_preview(p_answers, p_education_context);
  if v_preview ->> 'status' not in ('candidate_complete', 'incomplete') then
    raise exception using errcode = '22023',
      message = 'SPMSQ trial preview could not be reproduced';
  end if;
  ...
  v_request_hash := encode(sha256(convert_to(jsonb_build_object(
    ...
    'education_context', p_education_context,
    'cultural_context', p_cultural_context,
    'rule_version_id', p_rule_version_id
  )::text, 'UTF8')), 'hex');

  perform pg_advisory_xact_lock(hashtextextended(
    'spmsq-operation:' || v_actor::text || ':' || p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.spmsq_assessment_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'SPMSQ assessment idempotency conflict';
    end if;
    if not private.spmsq_client_authority(
      p_expected_organization_id, p_expected_branch_id,
      v_operation.client_id, 'assessments.manage'
    ) then
      raise exception using errcode = '42501',
        message = 'SPMSQ assessment replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.spmsq_assessment_versions version_row
    ...
    return query select v_operation.id, v_result.client_id,
      ...
      v_result.preview_status, v_result.preview_raw_errors,
      v_result.preview_adjusted_errors, v_result.preview_band_key,
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'spmsq-chain:' || p_expected_organization_id::text || ':' ||
  ...
  if p_action = 'revise_draft' then
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and not exists (
        select 1 from public.spmsq_assessment_versions child
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'SPMSQ candidate assessment version is stale';
    end if;
  end if;
  ...
  if v_author_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'SPMSQ author identity or client status is invalid';
  end if;
  ...
  v_content_hash := encode(sha256(convert_to(jsonb_build_object(
    ...
    'answers', p_answers, 'education_context', p_education_context,
    'cultural_context', p_cultural_context,
    'rule_version_id', p_rule_version_id,
  ...
  if not private.spmsq_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'SPMSQ assessment authority expired';
  end if;

  insert into public.spmsq_assessment_versions (
    ...
    education_context, cultural_context, rule_version_id, rule_snapshot,
    rule_snapshot_hash, governance_status, preview_status,
    preview_raw_errors, preview_adjusted_errors, preview_band_key,
    ...
    p_answers, p_education_context, p_cultural_context, p_rule_version_id,
    v_rule_snapshot, v_rule_hash, 'candidate_unactivated',
    v_preview ->> 'status', (v_preview ->> 'raw_errors')::integer,
    (v_preview ->> 'adjusted_errors')::integer,
    v_preview ->> 'band_key', v_challenge_id, v_content_hash, v_now
  ) returning * into v_result;

  insert into private.spmsq_assessment_operations (
  ...
  insert into public.audit_events (
    ...
    'spmsq_assessment_versions', v_result.id::text, p_idempotency_key,
    array[
      'record_state', 'version', 'assessed_on',
      'service_status_at_assessment', 'answers', 'education_context',
      'cultural_context', 'rule_version_id', 'rule_snapshot',
      'preview_status'
    ],
    jsonb_build_object(
      'workflow', 'page11_spmsq_candidate_v1',
  ...
  if not private.spmsq_client_authority(
    ...
    select 1 from public.spmsq_assessment_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'SPMSQ assessment final verification failed';
  end if;
...
create or replace function public.create_spmsq_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessed_on date,
...
returns table(
  ...
  preview_raw_errors integer, preview_adjusted_errors integer,
  preview_band_key text, committed_at timestamptz, replayed boolean
)
language sql volatile security invoker set search_path = ''
as $$
  select * from private.mutate_spmsq_assessment_guarded(
    'create_draft', p_expected_organization_id, p_expected_branch_id,
    p_client_id, null, null, 0, p_assessed_on, p_answers,
    p_education_context, p_cultural_context, p_rule_version_id,
    p_idempotency_key
  );
$$;

create or replace function public.revise_spmsq_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
as $$
  select * from private.mutate_spmsq_assessment_guarded(
    ...
    p_expected_version, p_assessed_on, p_answers, p_education_context,
    p_cultural_context, p_rule_version_id, p_idempotency_key
  );
$$;

create or replace function public.sign_spmsq_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
begin
  raise exception using errcode = '55000',
    message = 'SPMSQ candidate rule is not activated; formal signing is blocked';
end;
$$;

create trigger spmsq_assessment_versions_append_only
before update or delete on public.spmsq_assessment_versions
for each row execute function private.spmsq_history_is_append_only();
create trigger spmsq_assessment_operations_append_only
before update or delete on private.spmsq_assessment_operations
for each row execute function private.spmsq_history_is_append_only();
create trigger spmsq_assessment_versions_audit_row_change
after insert on public.spmsq_assessment_versions
for each row execute function private.audit_row_change();
create trigger spmsq_assessment_operations_audit_row_change
after insert on private.spmsq_assessment_operations
for each row execute function private.audit_row_change();

create or replace function private.spmsq_assessment_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_preview_filter text,
  p_education_filter text,
  p_reference_time timestamptz
)
returns jsonb
language sql
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(version_row.client_id, 'assessments.read')
      and not exists (
        select 1 from public.spmsq_assessment_versions child
        where child.previous_version_id = version_row.id
      )
  ...
  assigned_clients as materialized (
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(client.id, 'assessments.read')
  ),
  ...
  matching as materialized (
    ...
    where (p_client_id is null or row.assigned_client_id = p_client_id)
      ...
      and (
        p_education_filter = 'all'
        or row.education_context ->> 'state' = p_education_filter
      )
  ),
  stats as (
  ...
  items_result as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'preview_raw_errors', row.preview_raw_errors,
      'preview_adjusted_errors', row.preview_adjusted_errors,
      'preview_band_key', row.preview_band_key,
      ...
      'version_history', case when row.assessment_key is null then '[]'::jsonb
          ...
          select jsonb_agg(jsonb_build_object(
            ...
            'education_context', history.education_context,
            ...
            'preview_raw_errors', history.preview_raw_errors,
            'preview_adjusted_errors', history.preview_adjusted_errors,
            'preview_band_key', history.preview_band_key,
      ...
      'version_history_total', case when row.assessment_key is null then 0
          ...
          from public.spmsq_assessment_versions history_count
...
create or replace function private.spmsq_assessment_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_preview_filter text,
...
returns table(
  ...
  cultural_adjustment_status text,
...
declare
  ...
  v_education_filter text := lower(
    coalesce(nullif(btrim(p_education_filter), ''), 'all')
  );
  v_bundle jsonb;
  v_after jsonb;
  v_fingerprint text;
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or v_education_filter not in (
       'all', 'answered', 'missing', 'not_applicable'
     )
     or not private.spmsq_current_authority(
       p_expected_organization_id, p_expected_branch_id, 'assessments.read'
     ) then
    raise exception using errcode = '42501',
      message = 'SPMSQ assessment snapshot is not permitted';
  end if;
  if p_client_id is not null and not private.spmsq_client_authority(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    'assessments.read'
  ) then
    raise exception using errcode = '42501',
      message = 'SPMSQ client filter is not permitted';
  end if;

  v_bundle := private.spmsq_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    v_preview_filter, v_education_filter, v_now
  );
  ...
  insert into public.audit_events (
    ...
    'spmsq_assessment_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page11_spmsq_candidate_v1',
  ...
  v_after := private.spmsq_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    v_preview_filter, v_education_filter, v_now
  );
  if not private.spmsq_current_authority(
    p_expected_organization_id, p_expected_branch_id, 'assessments.read'
  ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'SPMSQ assessment snapshot final verification failed';
  end if;
  ...
  return query select
    ...
    v_bundle ->> 'cultural_adjustment_status',
...
create or replace function public.spmsq_assessment_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid default null,
  p_preview_filter text default 'all',
  p_education_filter text default 'all'
)
returns table(
  ...
  formal_score_status text, care_decision_status text,
  cultural_adjustment_status text, attachment_status text,
  export_status text, offline_sync_status text
)
language sql volatile security invoker set search_path = ''
as $$
  select * from private.spmsq_assessment_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_preview_filter, p_education_filter
  );
$$;

create policy spmsq_assessment_versions_staff_select
on public.spmsq_assessment_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(client_id, 'assessments.read')
);

revoke all on function private.spmsq_candidate_rule_snapshot()
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_answers_are_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_education_context_is_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_cultural_context_is_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_trial_preview(jsonb,jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_spmsq_reauth_evidence(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_spmsq_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,jsonb,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_assessment_snapshot_bundle(uuid,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.spmsq_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_spmsq_assessment_draft(uuid,uuid,uuid,date,jsonb,jsonb,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_spmsq_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,jsonb,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_spmsq_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.spmsq_assessment_snapshot(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_spmsq_assessment_draft(uuid,uuid,uuid,date,jsonb,jsonb,jsonb,text,uuid)
  to authenticated;
grant execute on function public.revise_spmsq_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,jsonb,jsonb,text,uuid)
  to authenticated;
grant execute on function public.sign_spmsq_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.spmsq_assessment_snapshot(uuid,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers require only their exact guarded entry point.
grant execute on function private.mutate_spmsq_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,jsonb,jsonb,text,uuid)
  to authenticated;
grant execute on function private.spmsq_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  to authenticated;

comment on function public.create_spmsq_assessment_draft(uuid,uuid,uuid,date,jsonb,jsonb,jsonb,text,uuid)
is 'Creates an assigned-client SPMSQ candidate-rule draft after recent AAL2; never creates an official result.';
comment on function public.sign_spmsq_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
is 'Fail-closed formal-signing boundary while the candidate rule remains unactivated.';
comment on function public.spmsq_assessment_snapshot(uuid,uuid,uuid,text,text)
is 'Page 11 assigned-client list and immutable candidate preview history; audit excludes answers, filter values, and personal data.';
