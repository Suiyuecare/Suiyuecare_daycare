-- Page 40: immutable physical-therapy service records.
--
-- This domain is intentionally separate from Page 34 assessments. A service
-- version may freeze the latest terminal Page 34 assessment as a read-only
...
insert into public.permissions (permission_key, description, risk_level) values
  ('physical_therapy_services.read',
    'Read assigned-client physical therapy service records', 2),
  ('physical_therapy_services.manage',
    'Create and revise assigned-client physical therapy service drafts', 2),
  ('physical_therapy_services.sign',
    'Sign or correct assigned-client physical therapy service records', 3)
...
where role.is_system
  ...
  and permission.permission_key = 'physical_therapy_services.read'
...
create or replace function private.physical_therapy_service_value_is_valid(
  p_value jsonb
)
returns boolean
language sql
...
create table public.physical_therapy_service_record_versions (
  ...
  constraint physical_therapy_service_client_scope_fkey
    foreign key (client_id, organization_id, branch_id)
    references public.clients(id, organization_id, branch_id) on delete restrict,
  constraint physical_therapy_service_id_scope_key unique (
    id, organization_id, branch_id, client_id, record_key
  ),
  constraint physical_therapy_service_chain_key unique (
    organization_id, branch_id, record_key, version
  ),
  constraint physical_therapy_service_previous_key unique (previous_version_id),
  constraint physical_therapy_service_previous_scope_fkey
    foreign key (
      previous_version_id, organization_id, branch_id, client_id, record_key
    ) references public.physical_therapy_service_record_versions (
      id, organization_id, branch_id, client_id, record_key
    ) on delete restrict,
  constraint physical_therapy_service_assessment_scope_fkey
    foreign key (
  ...
  constraint physical_therapy_service_version_check check (
    version > 0 and (
      (version = 1 and previous_version_id is null)
      or (version > 1 and previous_version_id is not null)
    )
  ),
  constraint physical_therapy_service_state_check check (
    record_state in ('draft', 'signed', 'corrected')
  ),
  constraint physical_therapy_service_time_check check (
    extract(year from occurred_at at time zone 'Asia/Taipei')
      between 2000 and 2200
    and occurred_at <= created_at + interval '5 minutes'
  ),
  constraint physical_therapy_service_content_check check (
    private.physical_therapy_service_value_is_valid(service_content)
  ),
  constraint physical_therapy_service_reaction_check check (
    private.physical_therapy_service_value_is_valid(client_reaction)
  ),
  constraint physical_therapy_service_recommendation_check check (
    private.physical_therapy_service_value_is_valid(recommendation)
  ),
  constraint physical_therapy_service_therapist_check check (
  ...
  constraint physical_therapy_service_status_check check (
    service_status_at_occurrence in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint physical_therapy_service_assessment_alignment_check check (
    (
  ...
  constraint physical_therapy_service_signature_alignment_check check (
    (
      record_state = 'draft'
  ...
  constraint physical_therapy_service_hash_check
    check (content_hash ~ '^[a-f0-9]{64}$')
);

comment on table public.physical_therapy_service_record_versions is
  'Immutable Page 40 service-record versions. Signed rows are superseded only by a reasoned, signed correction.';
comment on column public.physical_therapy_service_record_versions.assessment_reference_version_id is
  'Database-selected read-only reference to the latest terminal Page 34 assessment effective at occurred_at; never browser-selected.';

create table private.physical_therapy_service_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  reauth_challenge_id uuid
    references private.reauth_challenges(id) on delete restrict,
  created_at timestamptz not null default clock_timestamp(),
  constraint physical_therapy_service_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint physical_therapy_service_operations_result_scope_fkey
    foreign key (
  ...
  constraint physical_therapy_service_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft', 'sign', 'correct')
  ),
  constraint physical_therapy_service_operations_hash_check
    check (request_hash ~ '^[a-f0-9]{64}$'),
  constraint physical_therapy_service_operations_version_check
    check (result_version > 0),
  constraint physical_therapy_service_operations_state_check
    check (result_state in ('draft', 'signed', 'corrected')),
  constraint physical_therapy_service_operations_reauth_check check (
...
create index physical_therapy_services_scope_occurred_idx
  on public.physical_therapy_service_record_versions (
    organization_id, branch_id, occurred_at desc, record_key, version desc
  );
create index physical_therapy_services_client_idx
  on public.physical_therapy_service_record_versions (
    client_id, occurred_at desc, record_key, version desc
  );
create index physical_therapy_services_previous_idx
  on public.physical_therapy_service_record_versions (previous_version_id)
  where previous_version_id is not null;
create index physical_therapy_services_therapist_idx
  on public.physical_therapy_service_record_versions (therapist_user_id);
create index physical_therapy_services_assessment_ref_idx
  on public.physical_therapy_service_record_versions (
    assessment_reference_version_id
  ) where assessment_reference_version_id is not null;
create index physical_therapy_services_assessment_key_idx
  on public.physical_therapy_service_record_versions (
    assessment_reference_key
  ) where assessment_reference_key is not null;
create index physical_therapy_services_signed_by_idx
  on public.physical_therapy_service_record_versions (signed_by)
  where signed_by is not null;
create index physical_therapy_services_reauth_idx
  on public.physical_therapy_service_record_versions (
    signature_reauth_challenge_id
  ) where signature_reauth_challenge_id is not null;
create index physical_therapy_service_operations_result_idx
  on private.physical_therapy_service_operations (result_version_id);
create index physical_therapy_service_operations_result_scope_idx
  on private.physical_therapy_service_operations (
    organization_id, branch_id, client_id, result_record_key
  );
create index physical_therapy_service_operations_reauth_idx
  on private.physical_therapy_service_operations (reauth_challenge_id)
  where reauth_challenge_id is not null;

alter table public.physical_therapy_service_record_versions
  enable row level security;
alter table public.physical_therapy_service_record_versions
  force row level security;
alter table private.physical_therapy_service_operations enable row level security;
alter table private.physical_therapy_service_operations force row level security;

revoke all on table public.physical_therapy_service_record_versions
  from public, anon, authenticated, service_role;
revoke all on table private.physical_therapy_service_operations
  from public, anon, authenticated, service_role;

create or replace function private.physical_therapy_service_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'physical therapy service history is append-only';
end;
$$;

create trigger physical_therapy_service_record_versions_append_only
before update or delete on public.physical_therapy_service_record_versions
for each row execute function private.physical_therapy_service_history_is_append_only();
create trigger physical_therapy_service_operations_append_only
before update or delete on private.physical_therapy_service_operations
for each row execute function private.physical_therapy_service_history_is_append_only();

create trigger physical_therapy_service_record_versions_audit_row_change
after insert on public.physical_therapy_service_record_versions
for each row execute function private.audit_row_change();
create trigger physical_therapy_service_operations_audit_row_change
after insert on private.physical_therapy_service_operations
for each row execute function private.audit_row_change();

create or replace function private.physical_therapy_service_current_authority(
  ...
  select auth.uid() is not null
    ...
    and exists (
      ...
      where profile.id = auth.uid()
        ...
        and (
          (p_permission = 'physical_therapy_services.read'
...
create or replace function private.physical_therapy_service_client_authority(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_permission text
...
set search_path = ''
as $$
  select private.physical_therapy_service_current_authority(
      p_expected_organization_id, p_expected_branch_id, p_permission
    )
    and exists (
    ...
    and private.can_staff_access_client(
      p_client_id, 'physical_therapy_services.read'
    )
    and (
      p_permission = 'physical_therapy_services.read'
...
create or replace function private.require_physical_therapy_service_reauth(
  p_actor uuid,
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'recent same-session AAL2 is required for physical therapy service signing';
  end if;
  begin
...
create or replace function private.mutate_physical_therapy_service_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_request_hash text;
  v_operation private.physical_therapy_service_operations%rowtype;
  v_previous public.physical_therapy_service_record_versions%rowtype;
  v_result public.physical_therapy_service_record_versions%rowtype;
  v_record_key uuid;
  v_version integer;
  v_state text;
...
begin
  if p_action not in ('create_draft', 'revise_draft', 'sign', 'correct')
     ...
     or not private.physical_therapy_service_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       case when p_action in ('sign', 'correct')
         then 'physical_therapy_services.sign'
         else 'physical_therapy_services.manage' end
     ) then
    raise exception using errcode = '42501',
      message = 'physical therapy service operation is not permitted';
  end if;
  ...
  if p_action in ('create_draft', 'revise_draft', 'correct') then
    if p_occurred_at is null
       ...
       or not private.physical_therapy_service_value_is_valid(p_service_content)
       or not private.physical_therapy_service_value_is_valid(p_client_reaction)
       or not private.physical_therapy_service_value_is_valid(p_recommendation)
      ...
      raise exception using errcode = '22023',
        message = 'physical therapy service content is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'physical therapy service signing must use the exact draft';
  end if;

  if p_action in ('sign', 'correct') then
    v_challenge_id := private.require_physical_therapy_service_reauth(
      v_actor, v_now
    );
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'physical-therapy-service-operation:' || v_actor::text || ':' ||
    p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.physical_therapy_service_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'physical therapy service idempotency conflict';
    end if;
    if not private.physical_therapy_service_client_authority(
      ...
      raise exception using errcode = '42501',
        message = 'physical therapy service replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.physical_therapy_service_record_versions version_row
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'physical-therapy-service-chain:' ||
  ...
  if p_action <> 'create_draft' then
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and not exists (
        select 1 from public.physical_therapy_service_record_versions child
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'physical therapy service version is stale';
    end if;
  end if;
  ...
  if p_action = 'create_draft' then
    ...
    if v_previous.record_state <> 'draft' then
      raise exception using errcode = '23514',
        message = 'signed physical therapy service content requires correction';
    end if;
    ...
    if v_previous.record_state <> 'draft' then
      raise exception using errcode = '23514',
        message = 'only the current physical therapy service draft can be signed';
    end if;
    if v_previous.therapist_user_id <> v_actor then
      raise exception using errcode = '42501',
        message = 'only the current physical therapist may sign this draft';
    end if;
    ...
    if v_previous.record_state not in ('signed', 'corrected') then
      raise exception using errcode = '23514',
        message = 'only signed physical therapy service content can be corrected';
    end if;
  ...
  if v_occurred_at > v_now + interval '5 minutes' then
    raise exception using errcode = '22023',
      message = 'future physical therapy service occurrence is invalid';
  end if;
  ...
  if v_therapist_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'physical therapist or service occurrence is not eligible';
  end if;
  ...
  if p_action in ('sign', 'correct') then
    ...
    if v_signer_display_name is null
      ...
      raise exception using errcode = '42501',
        message = 'physical therapy service signer identity is invalid';
    end if;
  end if;
  ...
  if not private.physical_therapy_service_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'physical therapy service authority expired';
  end if;
  ...
  insert into public.physical_therapy_service_record_versions (
    ...
    case when v_state = 'signed' then '物理治療服務紀錄簽署'
      when v_state = 'corrected' then '物理治療服務紀錄更正簽署'
      else null end,
    v_challenge_id, v_content_hash, v_now
  ) returning * into v_result;

  insert into private.physical_therapy_service_operations (
  ...
  insert into public.audit_events (
    ...
    'physical_therapy_service_record_versions', v_result.id::text,
    p_idempotency_key,
    array['record_state', 'version', 'occurred_at', 'service_content',
      'client_reaction', 'recommendation',
      'assessment_reference_version_id'],
    jsonb_build_object(
      'workflow', 'page40_physical_therapy_service_v1',
  ...
  if not private.physical_therapy_service_client_authority(
     ...
     or not exists (
       select 1 from public.physical_therapy_service_record_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'physical therapy service final verification failed';
  end if;
...
create or replace function public.create_physical_therapy_service_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_occurred_at timestamptz,
...
as $$
  select * from private.mutate_physical_therapy_service_guarded(
...
create or replace function public.revise_physical_therapy_service_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_record_key uuid,
...
create or replace function public.sign_physical_therapy_service_record(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_record_key uuid,
...
create or replace function public.correct_physical_therapy_service_record(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_record_key uuid,
...
create or replace function private.physical_therapy_service_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_date_from date,
  p_date_to date,
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(
        version_row.client_id, 'physical_therapy_services.read'
      )
      and not exists (
        select 1
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'version_history', coalesce((
        ...
        from (
          select history.*
          from public.physical_therapy_service_record_versions history
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(
        client.id, 'physical_therapy_services.read'
      )
  ), client_ranked as (
...
create or replace function private.physical_therapy_service_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_date_from date,
  p_date_to date,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or not private.physical_therapy_service_current_authority(
       p_expected_organization_id, p_expected_branch_id,
       'physical_therapy_services.read'
     ) then
    raise exception using errcode = '42501',
      message = 'physical therapy service snapshot is not permitted';
  end if;

  if p_client_id is not null and
     not private.physical_therapy_service_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'physical_therapy_services.read'
     ) then
    raise exception using errcode = '42501',
      message = 'physical therapy service client filter is not permitted';
  end if;
  ...
  if p_therapist_user_id is not null and not exists (
    ...
    raise exception using errcode = '42501',
      message = 'physical therapy service therapist filter is not permitted';
  end if;

  v_bundle := private.physical_therapy_service_snapshot_bundle(
  ...
  insert into public.audit_events (
    ...
    'physical_therapy_service_record_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page40_physical_therapy_service_v1',
  ...
  v_after := private.physical_therapy_service_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_date_from,
    p_date_to, p_client_id, p_therapist_user_id, v_state, v_keyword, v_now
  );
  if not private.physical_therapy_service_current_authority(
       p_expected_organization_id, p_expected_branch_id,
       'physical_therapy_services.read'
     ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'physical therapy service snapshot final verification failed';
  end if;
...
create or replace function public.physical_therapy_service_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
...
set search_path = ''
as $$
  select * from private.physical_therapy_service_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_date_from,
    p_date_to, p_client_id, p_therapist_user_id, p_record_state, p_keyword
  );
$$;

create policy physical_therapy_service_record_versions_staff_select
on public.physical_therapy_service_record_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(
    client_id, 'physical_therapy_services.read'
  )
);

revoke all on function private.physical_therapy_service_value_is_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.physical_therapy_service_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.physical_therapy_service_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.physical_therapy_service_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_physical_therapy_service_reauth(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_physical_therapy_service_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.physical_therapy_service_snapshot_bundle(uuid,uuid,date,date,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.physical_therapy_service_snapshot_response(uuid,uuid,date,date,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_physical_therapy_service_draft(uuid,uuid,uuid,timestamptz,jsonb,jsonb,jsonb,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_physical_therapy_service_draft(uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_physical_therapy_service_record(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.correct_physical_therapy_service_record(uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.physical_therapy_service_snapshot(uuid,uuid,date,date,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_physical_therapy_service_draft(uuid,uuid,uuid,timestamptz,jsonb,jsonb,jsonb,uuid)
  to authenticated;
grant execute on function public.revise_physical_therapy_service_draft(uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,uuid)
  to authenticated;
grant execute on function public.sign_physical_therapy_service_record(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.correct_physical_therapy_service_record(uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,text,uuid)
  to authenticated;
grant execute on function public.physical_therapy_service_snapshot(uuid,uuid,date,date,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers receive only their exact private entry points.
grant execute on function private.mutate_physical_therapy_service_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,timestamptz,jsonb,jsonb,jsonb,text,uuid)
  to authenticated;
grant execute on function private.physical_therapy_service_snapshot_response(uuid,uuid,date,date,uuid,uuid,text,text)
  to authenticated;

comment on function public.physical_therapy_service_snapshot(uuid,uuid,date,date,uuid,uuid,text,text)
is 'Page 40 assigned-client physical therapy service records. Page 34 is a database-selected read-only reference; no formula, diagnosis, or automatic recommendation is inferred.';
