-- Page 13: manual, unstandardized fall-risk candidate drafts.
--
...
insert into public.permissions (permission_key, description, risk_level) values
  ('fall_risk_assessments.read', 'Read assigned-client Fall risk candidate drafts', 2),
  ('fall_risk_assessments.manage', 'Create and revise assigned-client Fall risk candidate drafts', 3),
  ('fall_risk_assessments.sign', 'Request governed Fall risk signing after rule activation', 3)
...
where role.is_system
  ...
  and permission.permission_key in (
    'fall_risk_assessments.read', 'fall_risk_assessments.manage', 'fall_risk_assessments.sign'
  )
on conflict (role_id, permission_id) do nothing;

create or replace function private.fall_risk_candidate_rule_snapshot()
returns jsonb
language sql
immutable
security invoker
...
as $$
  select jsonb_build_object(
    ...
    'factor_definitions', jsonb_build_array(
      jsonb_build_object('id', 'fall_factor_01', 'label', '人工觀察：近期跌倒或近跌事件', 'candidate_weight', 1),
      jsonb_build_object('id', 'fall_factor_02', 'label', '人工觀察：行走或移位穩定情形', 'candidate_weight', 1),
      jsonb_build_object('id', 'fall_factor_03', 'label', '人工觀察：輔具或他人協助需求', 'candidate_weight', 1),
      jsonb_build_object('id', 'fall_factor_04', 'label', '人工觀察：暈眩、嗜睡或姿勢改變反應', 'candidate_weight', 1),
      jsonb_build_object('id', 'fall_factor_05', 'label', '人工觀察：環境或活動情境風險', 'candidate_weight', 1),
      jsonb_build_object('id', 'fall_factor_06', 'label', '人工觀察：安全指令理解與遵循情形', 'candidate_weight', 1)
    ),
    'answer_values', jsonb_build_array('present', 'absent'),
    'candidate_present_item_ids', jsonb_build_array(
      'fall_factor_01', 'fall_factor_02', 'fall_factor_03',
      'fall_factor_04', 'fall_factor_05', 'fall_factor_06'
    ),
    ...
    'candidate_bands', jsonb_build_array(
      jsonb_build_object('key', 'candidate_observation_0_1', 'min', 0, 'max', 1),
      jsonb_build_object('key', 'candidate_review_2_3', 'min', 2, 'max', 3),
      jsonb_build_object('key', 'candidate_high_review_4_6', 'min', 4, 'max', 6)
    ),
...
create or replace function private.fall_risk_answers_are_valid(p_answers jsonb)
returns boolean
language sql
immutable
security invoker
...
create or replace function private.fall_risk_trial_preview(
  p_answers jsonb
)
returns jsonb
language plpgsql
...
declare
  ...
  v_band text;
begin
  if not private.fall_risk_answers_are_valid(p_answers) then
    return jsonb_build_object(
      'status', 'invalid', 'candidate_points', null, 'band_key', null
    );
  end if;
  ...
  v_band := case
    when v_raw between 0 and 1 then 'candidate_observation_0_1'
    when v_raw between 2 and 3 then 'candidate_review_2_3'
    else 'candidate_high_review_4_6'
  end;
  return jsonb_build_object(
    'status', 'candidate_complete', 'candidate_points', v_raw,
    'band_key', v_band
  );
end;
$$;

create table public.fall_risk_assessment_versions (
  ...
  preview_candidate_points integer,
  ...
  constraint fall_risk_assessment_client_scope_fkey
    foreign key (client_id, organization_id, branch_id)
    references public.clients(id, organization_id, branch_id) on delete restrict,
  constraint fall_risk_assessment_id_scope_key unique (
    id, organization_id, branch_id, client_id, assessment_key
  ),
  constraint fall_risk_assessment_chain_key unique (
    organization_id, branch_id, assessment_key, version
  ),
  constraint fall_risk_assessment_previous_key unique (previous_version_id),
  constraint fall_risk_assessment_previous_scope_fkey
    foreign key (
      previous_version_id, organization_id, branch_id, client_id,
      assessment_key
    ) references public.fall_risk_assessment_versions (
      id, organization_id, branch_id, client_id, assessment_key
    ) on delete restrict,
  constraint fall_risk_assessment_version_check check (
    version > 0 and (
      (version = 1 and previous_version_id is null)
      or (version > 1 and previous_version_id is not null)
    )
  ),
  constraint fall_risk_assessment_state_check check (
    record_state = 'draft_preview'
  ),
  constraint fall_risk_assessment_date_check check (
    extract(year from assessed_on) between 2000 and 2200
  ),
  constraint fall_risk_assessment_author_check check (
  ...
  constraint fall_risk_assessment_service_status_check check (
    service_status_at_assessment in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint fall_risk_assessment_answers_check check (
    private.fall_risk_answers_are_valid(answers)
  ),
  constraint fall_risk_assessment_rule_check check (
    rule_version_id = 'fall-risk-manual-factors-candidate-v1'
    and governance_status = 'candidate_unactivated'
    and rule_snapshot = private.fall_risk_candidate_rule_snapshot()
    and rule_snapshot_hash = encode(
      sha256(convert_to(rule_snapshot::text, 'UTF8')), 'hex'
    )
  ),
  constraint fall_risk_assessment_preview_check check (
    preview_status = private.fall_risk_trial_preview(answers) ->> 'status'
    and preview_candidate_points is not distinct from (
      private.fall_risk_trial_preview(answers)
        ->> 'candidate_points'
    )::integer
    and preview_band_key is not distinct from
      private.fall_risk_trial_preview(answers) ->> 'band_key'
    and preview_status in ('candidate_complete', 'incomplete')
  ),
  constraint fall_risk_assessment_hash_check check (
    content_hash ~ '^[a-f0-9]{64}$'
  )
);

comment on table public.fall_risk_assessment_versions is
  'Immutable Page 13 manual candidate drafts. Unactivated previews cannot be signed or used as official fall-risk results.';
comment on column public.fall_risk_assessment_versions.answers is
  'Six governed manual factors with explicit answered, missing, or not-applicable state; values are never written to audit metadata.';

create table private.fall_risk_assessment_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  reauth_challenge_id uuid not null
    references private.reauth_challenges(id) on delete restrict,
  created_at timestamptz not null default clock_timestamp(),
  constraint fall_risk_assessment_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint fall_risk_assessment_operations_result_scope_fkey
    foreign key (
  ...
  constraint fall_risk_assessment_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft')
  ),
  constraint fall_risk_assessment_operations_request_hash_check check (
    request_hash ~ '^[a-f0-9]{64}$'
  ),
  constraint fall_risk_assessment_operations_result_check check (
    result_version > 0 and result_state = 'draft_preview'
  )
);

create index fall_risk_assessments_scope_latest_idx
  on public.fall_risk_assessment_versions (
    organization_id, branch_id, client_id, assessed_on desc,
    created_at desc, assessment_key, version desc
  );
create index fall_risk_assessments_chain_idx
  on public.fall_risk_assessment_versions (
    organization_id, branch_id, assessment_key, version desc
  );
create index fall_risk_assessments_previous_idx
  on public.fall_risk_assessment_versions (previous_version_id)
  where previous_version_id is not null;
create index fall_risk_assessments_author_idx
  on public.fall_risk_assessment_versions (author_user_id);
create index fall_risk_assessments_reauth_idx
  on public.fall_risk_assessment_versions (write_reauth_challenge_id);
create index fall_risk_operations_result_idx
  on private.fall_risk_assessment_operations (
    organization_id, branch_id, client_id, result_assessment_key,
    result_version_id
  );
create index fall_risk_operations_reauth_idx
  on private.fall_risk_assessment_operations (reauth_challenge_id);

alter table public.fall_risk_assessment_versions enable row level security;
alter table public.fall_risk_assessment_versions force row level security;
alter table private.fall_risk_assessment_operations enable row level security;
alter table private.fall_risk_assessment_operations force row level security;

revoke all on table public.fall_risk_assessment_versions
  from public, anon, authenticated, service_role;
revoke all on table private.fall_risk_assessment_operations
  from public, anon, authenticated, service_role;

create or replace function private.fall_risk_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'Fall risk candidate assessment history is append-only';
end;
$$;
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'current same-session recent AAL2 evidence is required for Fall risk draft writes';
  end if;
  begin
...
create or replace function private.mutate_fall_risk_assessment_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_request_hash text;
  v_operation private.fall_risk_assessment_operations%rowtype;
  v_previous public.fall_risk_assessment_versions%rowtype;
  v_result public.fall_risk_assessment_versions%rowtype;
  v_assessment_key uuid;
  v_version integer;
  v_author_display_name text;
  v_service_status text;
  v_rule_snapshot jsonb := private.fall_risk_candidate_rule_snapshot();
  v_rule_hash text;
  v_preview jsonb;
  v_challenge_id uuid;
  v_content_hash text;
begin
  if p_action not in ('create_draft', 'revise_draft')
     ...
     or not private.fall_risk_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'fall_risk_assessments.read'
     )
     or not private.fall_risk_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'fall_risk_assessments.manage'
     ) then
    raise exception using errcode = '42501',
      message = 'Fall risk candidate assessment operation is not permitted';
  end if;

  v_challenge_id := private.require_fall_risk_reauth_evidence(v_actor, v_now);
  ...
  if p_action = 'create_draft' then
    if p_assessment_key is not null or p_previous_version_id is not null
      ...
      raise exception using errcode = '22023',
        message = 'new Fall risk assessment chain input is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'Fall risk assessment chain input is invalid';
  end if;
  ...
  if p_assessed_on is null
     ...
     or p_rule_version_id <> 'fall-risk-manual-factors-candidate-v1'
     or not private.fall_risk_answers_are_valid(p_answers) then
    raise exception using errcode = '22023',
      message = 'Fall risk candidate assessment payload is invalid';
  end if;

  v_preview := private.fall_risk_trial_preview(p_answers);
  if v_preview ->> 'status' not in ('candidate_complete', 'incomplete') then
    raise exception using errcode = '22023',
      message = 'Fall risk trial preview could not be reproduced';
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'fall_risk-operation:' || v_actor::text || ':' || p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.fall_risk_assessment_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'Fall risk assessment idempotency conflict';
    end if;
    if not private.fall_risk_client_authority(
      p_expected_organization_id, p_expected_branch_id,
      v_operation.client_id, 'fall_risk_assessments.manage'
    ) then
      raise exception using errcode = '42501',
        message = 'Fall risk assessment replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.fall_risk_assessment_versions version_row
    ...
    return query select v_operation.id, v_result.client_id,
      ...
      v_result.preview_status, v_result.preview_candidate_points,
      v_result.preview_band_key, v_result.content_hash,
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'fall_risk-chain:' || p_expected_organization_id::text || ':' ||
  ...
  if p_action = 'revise_draft' then
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and not exists (
        select 1 from public.fall_risk_assessment_versions child
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'Fall risk candidate assessment version is stale';
    end if;
  end if;
  ...
  if v_author_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'Fall risk author identity or client status is invalid';
  end if;
  ...
  if not private.fall_risk_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'Fall risk assessment authority expired';
  end if;

  insert into public.fall_risk_assessment_versions (
    ...
    preview_candidate_points, preview_band_key,
    ...
    v_preview ->> 'status', (v_preview ->> 'candidate_points')::integer,
    v_preview ->> 'band_key', v_challenge_id, v_content_hash, v_now
  ) returning * into v_result;

  insert into private.fall_risk_assessment_operations (
  ...
  insert into public.audit_events (
    ...
    'fall_risk_assessment_versions', v_result.id::text, p_idempotency_key,
    array[
  ...
  if not private.fall_risk_client_authority(
    ...
    select 1 from public.fall_risk_assessment_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'Fall risk assessment final verification failed';
  end if;
...
create or replace function public.create_fall_risk_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessed_on date,
...
returns table(
  ...
  preview_candidate_points integer,
  preview_band_key text, content_hash text,
  committed_at timestamptz, replayed boolean
)
language sql volatile security invoker set search_path = ''
as $$
  select * from private.mutate_fall_risk_assessment_guarded(
...
create or replace function public.revise_fall_risk_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create or replace function private.block_fall_risk_signing_guarded(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
    ...
    raise exception using errcode = '42501',
      message = 'Fall risk formal signing request is not permitted';
  end if;
  perform private.require_fall_risk_reauth_evidence(v_actor, clock_timestamp());
  raise exception using errcode = '55000',
    message = 'Fall risk candidate rule is not activated; formal signing is blocked';
end;
$$;

create or replace function public.sign_fall_risk_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
set search_path = ''
as $$
  select * from private.block_fall_risk_signing_guarded(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
...
create trigger fall_risk_assessment_versions_append_only
before update or delete on public.fall_risk_assessment_versions
for each row execute function private.fall_risk_history_is_append_only();
create trigger fall_risk_assessment_operations_append_only
before update or delete on private.fall_risk_assessment_operations
for each row execute function private.fall_risk_history_is_append_only();
create trigger fall_risk_assessment_versions_audit_row_change
after insert on public.fall_risk_assessment_versions
for each row execute function private.audit_row_change();
create trigger fall_risk_assessment_operations_audit_row_change
after insert on private.fall_risk_assessment_operations
for each row execute function private.audit_row_change();

create or replace function private.fall_risk_assessment_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(version_row.client_id, 'fall_risk_assessments.read')
      and not exists (
        select 1 from public.fall_risk_assessment_versions child
        where child.previous_version_id = version_row.id
      )
  ...
  assigned_clients as materialized (
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(client.id, 'fall_risk_assessments.read')
  ),
  ...
  items_result as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'preview_candidate_points', row.preview_candidate_points,
      ...
      'version_history', case when row.assessment_key is null then '[]'::jsonb
          ...
          select jsonb_agg(jsonb_build_object(
            ...
            'preview_candidate_points', history.preview_candidate_points,
      ...
      'version_history_total', case when row.assessment_key is null then 0
          ...
          from public.fall_risk_assessment_versions history_count
...
create or replace function private.fall_risk_assessment_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_preview_filter text,
...
returns table(
  ...
  formal_risk_status text,
  care_decision_status text,
  draft_task_suggestion_status text,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or not private.fall_risk_current_authority(
       p_expected_organization_id, p_expected_branch_id, 'fall_risk_assessments.read'
     ) then
    raise exception using errcode = '42501',
      message = 'Fall risk assessment snapshot is not permitted';
  end if;
  if p_client_id is not null and not private.fall_risk_client_authority(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    'fall_risk_assessments.read'
  ) then
    raise exception using errcode = '42501',
      message = 'Fall risk client filter is not permitted';
  end if;

  v_bundle := private.fall_risk_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
  ...
  insert into public.audit_events (
    ...
    'fall_risk_assessment_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page13_fall_risk_candidate_v1',
  ...
  v_after := private.fall_risk_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    v_preview_filter, v_answer_filter, v_now
  );
  if not private.fall_risk_current_authority(
    p_expected_organization_id, p_expected_branch_id, 'fall_risk_assessments.read'
  ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'Fall risk assessment snapshot final verification failed';
  end if;
  ...
  return query select
    ...
    v_bundle ->> 'formal_risk_status',
    v_bundle ->> 'care_decision_status',
    v_bundle ->> 'draft_task_suggestion_status',
...
create or replace function public.fall_risk_assessment_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
...
returns table(
  ...
  formal_score_status text, formal_risk_status text,
  care_decision_status text, draft_task_suggestion_status text,
  attachment_status text,
  export_status text, offline_sync_status text, notification_status text
)
language sql volatile security invoker set search_path = ''
as $$
  select * from private.fall_risk_assessment_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_preview_filter, p_answer_filter
  );
$$;

create policy fall_risk_assessment_versions_staff_select
on public.fall_risk_assessment_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(client_id, 'fall_risk_assessments.read')
);

revoke all on function private.fall_risk_candidate_rule_snapshot()
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_answers_are_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_trial_preview(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_fall_risk_reauth_evidence(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.block_fall_risk_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_fall_risk_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_assessment_snapshot_bundle(uuid,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.fall_risk_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_fall_risk_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_fall_risk_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_fall_risk_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.fall_risk_assessment_snapshot(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_fall_risk_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.revise_fall_risk_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.sign_fall_risk_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.fall_risk_assessment_snapshot(uuid,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers require only their exact guarded entry point.
grant execute on function private.mutate_fall_risk_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function private.block_fall_risk_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function private.fall_risk_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  to authenticated;

comment on function public.create_fall_risk_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
is 'Creates an assigned-client Fall risk candidate-rule draft after recent AAL2; never creates an official result.';
comment on function public.sign_fall_risk_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
is 'Fail-closed formal-signing boundary while the candidate rule remains unactivated.';
comment on function public.fall_risk_assessment_snapshot(uuid,uuid,uuid,text,text)
is 'Page 13 assigned-client list and immutable candidate preview history; audit excludes answers, filter values, and personal data.';
