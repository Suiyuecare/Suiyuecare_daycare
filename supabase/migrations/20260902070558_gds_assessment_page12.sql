-- Page 12: GDS candidate-rule drafts and reproducible trial previews.
--
...
insert into public.permissions (permission_key, description, risk_level) values
  ('gds_assessments.read', 'Read assigned-client GDS candidate drafts', 2),
  ('gds_assessments.manage', 'Create and revise assigned-client GDS candidate drafts', 3),
  ('gds_assessments.sign', 'Request governed GDS signing after rule activation', 3)
...
where role.is_system
  ...
  and permission.permission_key in (
    'gds_assessments.read', 'gds_assessments.manage', 'gds_assessments.sign'
  )
on conflict (role_id, permission_id) do nothing;

create or replace function private.gds_candidate_rule_snapshot()
returns jsonb
language sql
immutable
security invoker
...
as $$
  select jsonb_build_object(
    ...
    'answer_values', jsonb_build_array('yes', 'no'),
    'scored_yes_item_ids', jsonb_build_array(
      'gds_02', 'gds_03', 'gds_04', 'gds_06', 'gds_08',
      'gds_09', 'gds_10', 'gds_12', 'gds_14', 'gds_15'
    ),
    'scored_no_item_ids', jsonb_build_array(
    ...
    'candidate_bands', jsonb_build_array(
      jsonb_build_object('key', 'reference_0_4', 'min', 0, 'max', 4),
      jsonb_build_object('key', 'elevated_5_8', 'min', 5, 'max', 8),
      jsonb_build_object('key', 'high_9_11', 'min', 9, 'max', 11),
      jsonb_build_object('key', 'very_high_12_15', 'min', 12, 'max', 15)
    ),
    'disclaimer', 'Candidate trial preview only; not a diagnosis, official result, signature, or care decision.'
  );
$$;

create or replace function private.gds_answers_are_valid(p_answers jsonb)
returns boolean
language sql
immutable
security invoker
...
as $$
  select jsonb_typeof(p_answers) = 'object'
    and (select count(*) from jsonb_object_keys(p_answers)) = 15
    and not exists (
      select 1
      ...
      where jsonb_typeof(entry.value) <> 'object'
        ...
        or (
          ...
          and (
            ...
            or entry.value ->> 'value' not in ('yes', 'no')
          )
        )
        or (
...
create or replace function private.gds_trial_preview(
  p_answers jsonb
)
returns jsonb
language plpgsql
...
declare
  ...
  v_band text;
begin
  if not private.gds_answers_are_valid(p_answers) then
  ...
  where (entry.key in (
      'gds_02', 'gds_03', 'gds_04', 'gds_06', 'gds_08',
      'gds_09', 'gds_10', 'gds_12', 'gds_14', 'gds_15'
    ) and entry.value ->> 'value' = 'yes')
    or (entry.key in (
      'gds_01', 'gds_05', 'gds_07', 'gds_11', 'gds_13'
    ) and entry.value ->> 'value' = 'no');
  v_band := case
    when v_raw between 0 and 4 then 'reference_0_4'
    when v_raw between 5 and 8 then 'elevated_5_8'
    when v_raw between 9 and 11 then 'high_9_11'
    else 'very_high_12_15'
  end;
  return jsonb_build_object(
...
create table public.gds_assessment_versions (
  ...
  constraint gds_assessment_client_scope_fkey
    foreign key (client_id, organization_id, branch_id)
    references public.clients(id, organization_id, branch_id) on delete restrict,
  constraint gds_assessment_id_scope_key unique (
    id, organization_id, branch_id, client_id, assessment_key
  ),
  constraint gds_assessment_chain_key unique (
    organization_id, branch_id, assessment_key, version
  ),
  constraint gds_assessment_previous_key unique (previous_version_id),
  constraint gds_assessment_previous_scope_fkey
    foreign key (
      previous_version_id, organization_id, branch_id, client_id,
      assessment_key
    ) references public.gds_assessment_versions (
      id, organization_id, branch_id, client_id, assessment_key
    ) on delete restrict,
  constraint gds_assessment_version_check check (
    version > 0 and (
      (version = 1 and previous_version_id is null)
      or (version > 1 and previous_version_id is not null)
    )
  ),
  constraint gds_assessment_state_check check (
    record_state = 'draft_preview'
  ),
  constraint gds_assessment_date_check check (
    extract(year from assessed_on) between 2000 and 2200
  ),
  constraint gds_assessment_author_check check (
  ...
  constraint gds_assessment_service_status_check check (
    service_status_at_assessment in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint gds_assessment_answers_check check (
    private.gds_answers_are_valid(answers)
  ),
  constraint gds_assessment_rule_check check (
    rule_version_id = 'gds-15-strict-complete-v1'
    and governance_status = 'candidate_unactivated'
    and rule_snapshot = private.gds_candidate_rule_snapshot()
    and rule_snapshot_hash = encode(
      sha256(convert_to(rule_snapshot::text, 'UTF8')), 'hex'
    )
  ),
  constraint gds_assessment_preview_check check (
    preview_status = private.gds_trial_preview(answers) ->> 'status'
    and preview_candidate_points is not distinct from (
      private.gds_trial_preview(answers)
        ->> 'candidate_points'
    )::integer
    and preview_band_key is not distinct from
      private.gds_trial_preview(answers) ->> 'band_key'
    and preview_status in ('candidate_complete', 'incomplete')
  ),
  constraint gds_assessment_hash_check check (
    content_hash ~ '^[a-f0-9]{64}$'
  )
);

comment on table public.gds_assessment_versions is
  'Immutable Page 12 candidate-rule drafts. Unactivated previews cannot be signed or used as official GDS results.';
comment on column public.gds_assessment_versions.answers is
  'All fifteen answer slots with explicit answered, missing, or not-applicable state; values are never written to audit metadata.';

create table private.gds_assessment_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  reauth_challenge_id uuid not null
    references private.reauth_challenges(id) on delete restrict,
  created_at timestamptz not null default clock_timestamp(),
  constraint gds_assessment_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint gds_assessment_operations_result_scope_fkey
    foreign key (
  ...
  constraint gds_assessment_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft')
  ),
  constraint gds_assessment_operations_request_hash_check check (
    request_hash ~ '^[a-f0-9]{64}$'
  ),
  constraint gds_assessment_operations_result_check check (
    result_version > 0 and result_state = 'draft_preview'
  )
);

create index gds_assessments_scope_latest_idx
  on public.gds_assessment_versions (
    organization_id, branch_id, client_id, assessed_on desc,
    created_at desc, assessment_key, version desc
  );
create index gds_assessments_chain_idx
  on public.gds_assessment_versions (
    organization_id, branch_id, assessment_key, version desc
  );
create index gds_assessments_previous_idx
  on public.gds_assessment_versions (previous_version_id)
  where previous_version_id is not null;
create index gds_assessments_author_idx
  on public.gds_assessment_versions (author_user_id);
create index gds_assessments_reauth_idx
  on public.gds_assessment_versions (write_reauth_challenge_id);
create index gds_operations_result_idx
  on private.gds_assessment_operations (
    organization_id, branch_id, client_id, result_assessment_key,
    result_version_id
  );
create index gds_operations_reauth_idx
  on private.gds_assessment_operations (reauth_challenge_id);

alter table public.gds_assessment_versions enable row level security;
alter table public.gds_assessment_versions force row level security;
alter table private.gds_assessment_operations enable row level security;
alter table private.gds_assessment_operations force row level security;

revoke all on table public.gds_assessment_versions
  from public, anon, authenticated, service_role;
revoke all on table private.gds_assessment_operations
  from public, anon, authenticated, service_role;

create or replace function private.gds_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'GDS candidate assessment history is append-only';
end;
$$;
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'current same-session recent AAL2 evidence is required for GDS draft writes';
  end if;
  begin
...
create or replace function private.mutate_gds_assessment_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_request_hash text;
  v_operation private.gds_assessment_operations%rowtype;
  v_previous public.gds_assessment_versions%rowtype;
  v_result public.gds_assessment_versions%rowtype;
  v_assessment_key uuid;
  v_version integer;
  v_author_display_name text;
  v_service_status text;
  v_rule_snapshot jsonb := private.gds_candidate_rule_snapshot();
  v_rule_hash text;
  v_preview jsonb;
  v_challenge_id uuid;
  v_content_hash text;
begin
  if p_action not in ('create_draft', 'revise_draft')
     ...
     or not private.gds_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'gds_assessments.read'
     )
     or not private.gds_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'gds_assessments.manage'
     ) then
    raise exception using errcode = '42501',
      message = 'GDS candidate assessment operation is not permitted';
  end if;

  v_challenge_id := private.require_gds_reauth_evidence(v_actor, v_now);
  ...
  if p_action = 'create_draft' then
    if p_assessment_key is not null or p_previous_version_id is not null
      ...
      raise exception using errcode = '22023',
        message = 'new GDS assessment chain input is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'GDS assessment chain input is invalid';
  end if;
  ...
  if p_assessed_on is null
     ...
     or p_rule_version_id <> 'gds-15-strict-complete-v1'
     or not private.gds_answers_are_valid(p_answers) then
    raise exception using errcode = '22023',
      message = 'GDS candidate assessment payload is invalid';
  end if;

  v_preview := private.gds_trial_preview(p_answers);
  if v_preview ->> 'status' not in ('candidate_complete', 'incomplete') then
    raise exception using errcode = '22023',
      message = 'GDS trial preview could not be reproduced';
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'gds-operation:' || v_actor::text || ':' || p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.gds_assessment_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'GDS assessment idempotency conflict';
    end if;
    if not private.gds_client_authority(
      p_expected_organization_id, p_expected_branch_id,
      v_operation.client_id, 'gds_assessments.manage'
    ) then
      raise exception using errcode = '42501',
        message = 'GDS assessment replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.gds_assessment_versions version_row
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'gds-chain:' || p_expected_organization_id::text || ':' ||
  ...
  if p_action = 'revise_draft' then
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and not exists (
        select 1 from public.gds_assessment_versions child
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'GDS candidate assessment version is stale';
    end if;
  end if;
  ...
  if v_author_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'GDS author identity or client status is invalid';
  end if;
  ...
  if not private.gds_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'GDS assessment authority expired';
  end if;

  insert into public.gds_assessment_versions (
  ...
  insert into private.gds_assessment_operations (
  ...
  insert into public.audit_events (
    ...
    'gds_assessment_versions', v_result.id::text, p_idempotency_key,
    array[
    ...
    jsonb_build_object(
      'workflow', 'page12_gds_candidate_v1',
  ...
  if not private.gds_client_authority(
    ...
    select 1 from public.gds_assessment_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'GDS assessment final verification failed';
  end if;
...
create or replace function public.create_gds_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessed_on date,
...
as $$
  select * from private.mutate_gds_assessment_guarded(
...
create or replace function public.revise_gds_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create or replace function private.block_gds_signing_guarded(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
    ...
    raise exception using errcode = '42501',
      message = 'GDS formal signing request is not permitted';
  end if;
  perform private.require_gds_reauth_evidence(v_actor, clock_timestamp());
  raise exception using errcode = '55000',
    message = 'GDS candidate rule is not activated; formal signing is blocked';
end;
$$;

create or replace function public.sign_gds_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
set search_path = ''
as $$
  select * from private.block_gds_signing_guarded(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
...
create trigger gds_assessment_versions_append_only
before update or delete on public.gds_assessment_versions
for each row execute function private.gds_history_is_append_only();
create trigger gds_assessment_operations_append_only
before update or delete on private.gds_assessment_operations
for each row execute function private.gds_history_is_append_only();
create trigger gds_assessment_versions_audit_row_change
after insert on public.gds_assessment_versions
for each row execute function private.audit_row_change();
create trigger gds_assessment_operations_audit_row_change
after insert on private.gds_assessment_operations
for each row execute function private.audit_row_change();

create or replace function private.gds_assessment_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(version_row.client_id, 'gds_assessments.read')
      and not exists (
        select 1 from public.gds_assessment_versions child
        where child.previous_version_id = version_row.id
      )
  ...
  assigned_clients as materialized (
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(client.id, 'gds_assessments.read')
  ),
  ...
  items_result as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'version_history_total', case when row.assessment_key is null then 0
          ...
          from public.gds_assessment_versions history_count
...
create or replace function private.gds_assessment_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_preview_filter text,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or not private.gds_current_authority(
       p_expected_organization_id, p_expected_branch_id, 'gds_assessments.read'
     ) then
    raise exception using errcode = '42501',
      message = 'GDS assessment snapshot is not permitted';
  end if;
  if p_client_id is not null and not private.gds_client_authority(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    'gds_assessments.read'
  ) then
    raise exception using errcode = '42501',
      message = 'GDS client filter is not permitted';
  end if;

  v_bundle := private.gds_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
  ...
  insert into public.audit_events (
    ...
    'gds_assessment_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page12_gds_candidate_v1',
  ...
  v_after := private.gds_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    v_preview_filter, v_answer_filter, v_now
  );
  if not private.gds_current_authority(
    p_expected_organization_id, p_expected_branch_id, 'gds_assessments.read'
  ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'GDS assessment snapshot final verification failed';
  end if;
...
create or replace function public.gds_assessment_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
...
returns table(
  ...
  care_decision_status text, attachment_status text,
  export_status text, offline_sync_status text, notification_status text
)
language sql volatile security invoker set search_path = ''
as $$
  select * from private.gds_assessment_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_preview_filter, p_answer_filter
  );
$$;

create policy gds_assessment_versions_staff_select
on public.gds_assessment_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(client_id, 'gds_assessments.read')
);

revoke all on function private.gds_candidate_rule_snapshot()
  from public, anon, authenticated, service_role;
revoke all on function private.gds_answers_are_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.gds_trial_preview(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.gds_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.gds_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.gds_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_gds_reauth_evidence(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.block_gds_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_gds_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.gds_assessment_snapshot_bundle(uuid,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.gds_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_gds_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_gds_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_gds_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.gds_assessment_snapshot(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_gds_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.revise_gds_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.sign_gds_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.gds_assessment_snapshot(uuid,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers require only their exact guarded entry point.
grant execute on function private.mutate_gds_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function private.block_gds_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function private.gds_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  to authenticated;

comment on function public.create_gds_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
is 'Creates an assigned-client GDS candidate-rule draft after recent AAL2; never creates an official result.';
comment on function public.sign_gds_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
is 'Fail-closed formal-signing boundary while the candidate rule remains unactivated.';
comment on function public.gds_assessment_snapshot(uuid,uuid,uuid,text,text)
is 'Page 12 assigned-client list and immutable candidate preview history; audit excludes answers, filter values, and personal data.';
