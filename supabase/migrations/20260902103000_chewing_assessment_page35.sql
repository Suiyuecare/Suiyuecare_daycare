-- Page 35: manual, unstandardized chewing candidate drafts.
--
...
insert into public.permissions (permission_key, description, risk_level) values
  ('chewing_assessments.read', 'Read assigned-client manual chewing observation candidate drafts', 2),
  ('chewing_assessments.manage', 'Create and revise assigned-client manual chewing observation candidate drafts', 3),
  ('chewing_assessments.sign', 'Request governed chewing assessment signing or correction after formal rule activation', 3)
...
as $$
  select jsonb_build_object(
    ...
    'field_ids', jsonb_build_array(
      'chewing_observation_01', 'chewing_observation_02', 'chewing_observation_03',
      'chewing_observation_04', 'chewing_observation_05', 'chewing_observation_06'
    ),
    'field_definitions', jsonb_build_array(
      jsonb_build_object('id', 'chewing_observation_01', 'label', '人工觀察：食物入口與口內處理情形需進一步確認', 'data_kind', 'manual_presence_observation'),
      jsonb_build_object('id', 'chewing_observation_02', 'label', '人工觀察：咀嚼動作或節律需進一步確認', 'data_kind', 'manual_presence_observation'),
      jsonb_build_object('id', 'chewing_observation_03', 'label', '人工觀察：口內食物殘留或清除情形需進一步確認', 'data_kind', 'manual_presence_observation'),
      jsonb_build_object('id', 'chewing_observation_04', 'label', '人工觀察：進食時疲勞或中止情形需進一步確認', 'data_kind', 'manual_presence_observation'),
      jsonb_build_object('id', 'chewing_observation_05', 'label', '人工觀察：牙齒、義齒或口腔不適情形需進一步確認', 'data_kind', 'manual_presence_observation'),
      jsonb_build_object('id', 'chewing_observation_06', 'label', '人工觀察：餐食質地或進食協助需求需專業覆核', 'data_kind', 'manual_presence_observation')
    ),
    ...
    'disclaimer', 'Manual unstandardized chewing observations only. The present count is not a score; no formal tool, licensed source, weights, ability classification, diagnosis, signature, correction, nutrition or swallowing referral, notification, or care decision is configured.'
  );
$$;

create or replace function private.chewing_answers_are_valid(p_answers jsonb)
returns boolean
language sql
immutable
security invoker
...
create or replace function private.chewing_trial_preview(
  p_answers jsonb
)
returns jsonb
language plpgsql
...
declare
  v_raw integer;
begin
  if not private.chewing_answers_are_valid(p_answers) then
...
create table public.chewing_assessment_versions (
  ...
  constraint chewing_assessment_client_scope_fkey
    foreign key (client_id, organization_id, branch_id)
    references public.clients(id, organization_id, branch_id) on delete restrict,
  constraint chewing_assessment_id_scope_key unique (
    id, organization_id, branch_id, client_id, assessment_key
  ),
  constraint chewing_assessment_chain_key unique (
    organization_id, branch_id, assessment_key, version
  ),
  constraint chewing_assessment_previous_key unique (previous_version_id),
  constraint chewing_assessment_previous_scope_fkey
    foreign key (
      previous_version_id, organization_id, branch_id, client_id,
      assessment_key
    ) references public.chewing_assessment_versions (
      id, organization_id, branch_id, client_id, assessment_key
    ) on delete restrict,
  constraint chewing_assessment_version_check check (
    version > 0 and (
      (version = 1 and previous_version_id is null)
      or (version > 1 and previous_version_id is not null)
    )
  ),
  constraint chewing_assessment_state_check check (
    record_state = 'draft_preview'
  ),
  constraint chewing_assessment_date_check check (
    extract(year from assessed_on) between 2000 and 2200
  ),
  constraint chewing_assessment_author_check check (
  ...
  constraint chewing_assessment_service_status_check check (
    service_status_at_assessment in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint chewing_assessment_answers_check check (
    private.chewing_answers_are_valid(answers)
  ),
  constraint chewing_assessment_rule_check check (
    rule_version_id = 'chewing-manual-observations-candidate-v1'
    and governance_status = 'candidate_unactivated'
    and rule_snapshot = private.chewing_candidate_rule_snapshot()
    and rule_snapshot_hash = encode(
      sha256(convert_to(rule_snapshot::text, 'UTF8')), 'hex'
    )
  ),
  constraint chewing_assessment_preview_check check (
    preview_status = private.chewing_trial_preview(answers) ->> 'status'
    and preview_observed_count is not distinct from (
      private.chewing_trial_preview(answers)
        ->> 'observed_count'
    )::integer
    and preview_status in ('candidate_complete', 'incomplete')
  ),
  constraint chewing_assessment_hash_check check (
    content_hash ~ '^[a-f0-9]{64}$'
  )
);

comment on table public.chewing_assessment_versions is
  'Immutable Page 35 manual chewing observation candidate drafts. Unactivated previews cannot be signed, corrected as formal records, or used as formal ability results.';
comment on column public.chewing_assessment_versions.answers is
...
create table private.chewing_assessment_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  reauth_challenge_id uuid not null
    references private.reauth_challenges(id) on delete restrict,
  created_at timestamptz not null default clock_timestamp(),
  constraint chewing_assessment_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint chewing_assessment_operations_result_scope_fkey
    foreign key (
  ...
  constraint chewing_assessment_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft')
  ),
  constraint chewing_assessment_operations_request_hash_check check (
    request_hash ~ '^[a-f0-9]{64}$'
  ),
  constraint chewing_assessment_operations_result_check check (
    result_version > 0 and result_state = 'draft_preview'
  )
);

create index chewing_assessments_scope_latest_idx
  on public.chewing_assessment_versions (
    organization_id, branch_id, client_id, assessed_on desc,
    created_at desc, assessment_key, version desc
  );
create index chewing_assessments_chain_idx
  on public.chewing_assessment_versions (
    organization_id, branch_id, assessment_key, version desc
  );
create index chewing_assessments_previous_idx
  on public.chewing_assessment_versions (previous_version_id)
  where previous_version_id is not null;
create index chewing_assessments_author_idx
  on public.chewing_assessment_versions (author_user_id);
create index chewing_assessments_reauth_idx
  on public.chewing_assessment_versions (write_reauth_challenge_id);
create index chewing_operations_result_idx
  on private.chewing_assessment_operations (
    organization_id, branch_id, client_id, result_assessment_key,
    result_version_id
  );
create index chewing_operations_reauth_idx
  on private.chewing_assessment_operations (reauth_challenge_id);

alter table public.chewing_assessment_versions enable row level security;
alter table public.chewing_assessment_versions force row level security;
alter table private.chewing_assessment_operations enable row level security;
alter table private.chewing_assessment_operations force row level security;

revoke all on table public.chewing_assessment_versions
  from public, anon, authenticated, service_role;
revoke all on table private.chewing_assessment_operations
  from public, anon, authenticated, service_role;

create or replace function private.chewing_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'manual chewing observation candidate history is append-only';
end;
$$;

create or replace function private.chewing_current_authority(
...
as $$
  select auth.uid() is not null
    ...
    and (
      p_permission = 'chewing_assessments.read'
      or exists (
...
create or replace function private.chewing_client_authority(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_permission text
...
set search_path = ''
as $$
  select private.chewing_current_authority(
      p_expected_organization_id, p_expected_branch_id, p_permission
    )
    and exists (
...
create or replace function private.require_chewing_reauth_evidence(
  p_actor uuid,
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'current same-session recent AAL2 evidence is required for manual chewing observation writes';
  end if;
  begin
...
create or replace function private.mutate_chewing_assessment_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_request_hash text;
  v_operation private.chewing_assessment_operations%rowtype;
  v_previous public.chewing_assessment_versions%rowtype;
  v_result public.chewing_assessment_versions%rowtype;
  v_assessment_key uuid;
  v_version integer;
  v_author_display_name text;
  v_service_status text;
  v_rule_snapshot jsonb := private.chewing_candidate_rule_snapshot();
  v_rule_hash text;
  v_preview jsonb;
  v_challenge_id uuid;
  v_content_hash text;
begin
  if p_action not in ('create_draft', 'revise_draft')
     ...
     or not private.chewing_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'chewing_assessments.read'
     )
     or not private.chewing_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'chewing_assessments.manage'
    ) then
    raise exception using errcode = '42501',
      message = 'manual chewing observation candidate operation is not permitted';
  end if;

  v_challenge_id := private.require_chewing_reauth_evidence(v_actor, v_now);
  ...
  if p_action = 'create_draft' then
    if p_assessment_key is not null or p_previous_version_id is not null
      ...
      raise exception using errcode = '22023',
        message = 'new manual chewing observation chain input is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'manual chewing observation chain input is invalid';
  end if;
  ...
  if p_assessed_on is null
     ...
     or p_rule_version_id <> 'chewing-manual-observations-candidate-v1'
     or not private.chewing_answers_are_valid(p_answers) then
    raise exception using errcode = '22023',
      message = 'manual chewing observation candidate payload is invalid';
  end if;

  v_preview := private.chewing_trial_preview(p_answers);
  if v_preview ->> 'status' not in ('candidate_complete', 'incomplete') then
    raise exception using errcode = '22023',
      message = 'manual chewing observation candidate replay could not be reproduced';
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'chewing-operation:' || v_actor::text || ':' || p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.chewing_assessment_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'manual chewing observation idempotency conflict';
    end if;
    if not private.chewing_client_authority(
      p_expected_organization_id, p_expected_branch_id,
      v_operation.client_id, 'chewing_assessments.manage'
    ) then
      raise exception using errcode = '42501',
        message = 'manual chewing observation replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.chewing_assessment_versions version_row
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'chewing-chain:' || p_expected_organization_id::text || ':' ||
  ...
  if p_action = 'revise_draft' then
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'manual chewing observation candidate version is stale';
    end if;
  end if;
  ...
  if v_author_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'manual chewing observation author identity or client status is invalid';
  end if;
  ...
  if not private.chewing_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'manual chewing observation authority expired';
  end if;

  insert into public.chewing_assessment_versions (
  ...
  insert into private.chewing_assessment_operations (
  ...
  insert into public.audit_events (
    ...
    'chewing_assessment_versions', v_result.id::text, p_idempotency_key,
    array[
    ...
    jsonb_build_object(
      'workflow', 'page35_chewing_candidate_v1',
      'version', v_result.version,
      'state', v_result.record_state,
      'candidate_unactivated', true,
      'formal_score_created', false,
      'formal_ability_classification_created', false,
      'diagnosis_created', false,
      'signature_created', false,
      'formal_correction_created', false,
      'nutrition_referral_created', false,
      'swallowing_referral_created', false,
  ...
  if not private.chewing_client_authority(
    ...
    select 1 from public.chewing_assessment_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'manual chewing observation final verification failed';
  end if;
...
create or replace function public.create_chewing_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessed_on date,
...
as $$
  select * from private.mutate_chewing_assessment_guarded(
...
create or replace function public.revise_chewing_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create or replace function private.block_chewing_signing_guarded(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
    ...
    raise exception using errcode = '42501',
      message = 'manual chewing observation formal signing request is not permitted';
  end if;
  perform private.require_chewing_reauth_evidence(v_actor, clock_timestamp());
  raise exception using errcode = '55000',
    message = 'formal chewing assessment tool, licensed source, weights, and ability classification are not activated; formal signing is blocked';
end;
$$;

create or replace function public.sign_chewing_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
set search_path = ''
as $$
  select * from private.block_chewing_signing_guarded(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
...
create or replace function private.block_chewing_correction_guarded(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
    ...
    raise exception using errcode = '42501',
      message = 'manual chewing observation formal correction request is not permitted';
  end if;
  perform private.require_chewing_reauth_evidence(v_actor, clock_timestamp());
  raise exception using errcode = '55000',
    message = 'no governed signed chewing assessment exists and the formal rule is not activated; correction is blocked';
end;
$$;

create or replace function public.correct_chewing_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
set search_path = ''
as $$
  select * from private.block_chewing_correction_guarded(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_assessment_key, p_previous_version_id, p_expected_version,
...
create trigger chewing_assessment_versions_append_only
before update or delete on public.chewing_assessment_versions
for each row execute function private.chewing_history_is_append_only();
create trigger chewing_assessment_operations_append_only
before update or delete on private.chewing_assessment_operations
for each row execute function private.chewing_history_is_append_only();
create trigger chewing_assessment_versions_audit_row_change
after insert on public.chewing_assessment_versions
for each row execute function private.audit_row_change();
create trigger chewing_assessment_operations_audit_row_change
after insert on private.chewing_assessment_operations
for each row execute function private.audit_row_change();

create or replace function private.chewing_assessment_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(version_row.client_id, 'chewing_assessments.read')
      and not exists (
        select 1 from public.chewing_assessment_versions child
        where child.previous_version_id = version_row.id
      )
  ...
  assigned_clients as materialized (
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(client.id, 'chewing_assessments.read')
  ),
  ...
  items_result as (
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'version_history_total', case when row.assessment_key is null then 0
          ...
          from public.chewing_assessment_versions history_count
...
create or replace function private.chewing_assessment_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_preview_filter text,
...
returns table(
  ...
  formal_correction_status text,
  formal_score_status text,
  formal_ability_classification_status text,
  diagnosis_status text,
  care_decision_status text,
  nutrition_referral_status text,
  swallowing_referral_status text,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or not private.chewing_current_authority(
       p_expected_organization_id, p_expected_branch_id, 'chewing_assessments.read'
    ) then
    raise exception using errcode = '42501',
      message = 'manual chewing observation snapshot is not permitted';
  end if;
  if p_client_id is not null and not private.chewing_client_authority(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    'chewing_assessments.read'
  ) then
    raise exception using errcode = '42501',
      message = 'manual chewing observation client filter is not permitted';
  end if;

  v_bundle := private.chewing_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
  ...
  insert into public.audit_events (
    ...
    'chewing_assessment_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page35_chewing_candidate_v1',
  ...
  v_after := private.chewing_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    v_preview_filter, v_answer_filter, v_now
  );
  if not private.chewing_current_authority(
    p_expected_organization_id, p_expected_branch_id, 'chewing_assessments.read'
  ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'manual chewing observation snapshot final verification failed';
  end if;
...
create or replace function public.chewing_assessment_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
...
returns table(
  ...
  formal_correction_status text,
  formal_score_status text, formal_ability_classification_status text,
  diagnosis_status text,
  care_decision_status text, nutrition_referral_status text,
  swallowing_referral_status text,
  attachment_status text,
...
language sql volatile security invoker set search_path = ''
as $$
  select * from private.chewing_assessment_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_preview_filter, p_answer_filter
  );
$$;

create policy chewing_assessment_versions_staff_select
on public.chewing_assessment_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(client_id, 'chewing_assessments.read')
);

revoke all on function private.chewing_candidate_rule_snapshot()
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_answers_are_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_trial_preview(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_chewing_reauth_evidence(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.block_chewing_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.block_chewing_correction_guarded(uuid,uuid,uuid,uuid,uuid,integer,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_chewing_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_assessment_snapshot_bundle(uuid,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.chewing_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_chewing_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_chewing_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.correct_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.chewing_assessment_snapshot(uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_chewing_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.revise_chewing_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function public.sign_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.correct_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,text,uuid)
  to authenticated;
grant execute on function public.chewing_assessment_snapshot(uuid,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers require only their exact guarded entry point.
grant execute on function private.mutate_chewing_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,jsonb,text,uuid)
  to authenticated;
grant execute on function private.block_chewing_signing_guarded(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function private.block_chewing_correction_guarded(uuid,uuid,uuid,uuid,uuid,integer,text,uuid)
  to authenticated;
grant execute on function private.chewing_assessment_snapshot_response(uuid,uuid,uuid,text,text)
  to authenticated;

comment on function public.create_chewing_assessment_draft(uuid,uuid,uuid,date,jsonb,text,uuid)
is 'Creates an assigned-client manual chewing observation candidate draft after recent AAL2; never creates a formal score or ability classification.';
comment on function public.sign_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
is 'Fail-closed formal-signing boundary while the candidate rule remains unactivated.';
comment on function public.correct_chewing_assessment(uuid,uuid,uuid,uuid,uuid,integer,text,uuid)
is 'Fail-closed formal-correction boundary while no governed signed record or activated rule exists.';
comment on function public.chewing_assessment_snapshot(uuid,uuid,uuid,text,text)
is 'Page 35 assigned-client list and immutable candidate preview history; audit excludes answers, filter values, and personal data.';
