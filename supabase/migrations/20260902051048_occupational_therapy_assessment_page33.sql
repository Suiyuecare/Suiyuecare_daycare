-- Page 33: manual, explicitly unstandardized occupational therapy assessments.
--
-- This release does not publish or imitate an official scale. It records an
-- assigned occupational therapist's dated observations, manually named
...
insert into public.permissions (permission_key, description, risk_level) values
  ('occupational_therapy_assessments.read',
    'Read assigned-client occupational therapy assessments', 2),
  ('occupational_therapy_assessments.manage',
    'Create and revise assigned-client occupational therapy assessment drafts', 2),
  ('occupational_therapy_assessments.sign',
    'Sign or correct assigned-client occupational therapy assessments', 3)
...
where role.is_system
  ...
  and permission.permission_key = 'occupational_therapy_assessments.read'
...
create or replace function private.occupational_therapy_measurements_are_valid(
...
create table public.occupational_therapy_assessment_versions (
  ...
  constraint occupational_therapy_assessment_client_scope_fkey
    foreign key (client_id, organization_id, branch_id)
    references public.clients(id, organization_id, branch_id) on delete restrict,
  constraint occupational_therapy_assessment_id_scope_key unique (
    id, organization_id, branch_id, client_id, assessment_key
  ),
  constraint occupational_therapy_assessment_chain_key unique (
    organization_id, branch_id, assessment_key, version
  ),
  constraint occupational_therapy_assessment_previous_key unique (previous_version_id),
  constraint occupational_therapy_assessment_previous_scope_fkey
    foreign key (
  ...
  constraint occupational_therapy_assessment_version_check check (
    version > 0 and (
      (version = 1 and previous_version_id is null)
      or (version > 1 and previous_version_id is not null)
    )
  ),
  constraint occupational_therapy_assessment_state_check check (
    record_state in ('draft', 'signed', 'corrected')
  ),
  constraint occupational_therapy_assessment_dates_check check (
  ...
  constraint occupational_therapy_assessment_service_status_check check (
    service_status_at_assessment in (
      'active', 'suspended', 'transferred', 'closed', 'deceased'
    )
  ),
  constraint occupational_therapy_assessment_therapist_check check (
  ...
  constraint occupational_therapy_assessment_due_basis_check check (
    char_length(due_basis) between 1 and 1000
    and due_basis = btrim(due_basis)
    and translate(due_basis, E'\n\r\t', '') !~ '[[:cntrl:]]'
  ),
  constraint occupational_therapy_assessment_measurements_check check (
    private.occupational_therapy_measurements_are_valid(measurements)
  ),
  constraint occupational_therapy_functional_observation_check check (
  ...
  constraint occupational_therapy_assessment_signature_alignment_check check (
    (
      ...
      and signature_purpose = case when record_state = 'signed'
        then '人工職能治療評估簽署' else '人工職能治療評估更正簽署' end
  ...
  constraint occupational_therapy_assessment_hash_check
    check (content_hash ~ '^[a-f0-9]{64}$')
);

comment on table public.occupational_therapy_assessment_versions is
...
comment on column public.occupational_therapy_assessment_versions.due_basis is
  'Staff-entered source or rationale for the manually entered due date.';
comment on column public.occupational_therapy_assessment_versions.form_version_reference is
  'Technical reference manual-occupational-therapy-v1; not an official or standardized scale.';
comment on column public.occupational_therapy_assessment_versions.measurements is
  'Manual named measurement items. Numeric values remain exact strings and are never summed into a score.';

create table private.occupational_therapy_assessment_operations (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null,
  branch_id uuid not null,
  client_id uuid not null,
  ...
  created_at timestamptz not null default clock_timestamp(),
  constraint occupational_therapy_assessment_operations_actor_key unique (
    actor_user_id, idempotency_key
  ),
  constraint occupational_therapy_assessment_operations_result_scope_fkey
    foreign key (
  ...
  constraint occupational_therapy_assessment_operations_kind_check check (
    operation_kind in ('create_draft', 'revise_draft', 'sign', 'correct')
  ),
  constraint occupational_therapy_assessment_operations_hash_check
    check (request_hash ~ '^[a-f0-9]{64}$'),
  constraint occupational_therapy_assessment_operations_version_check
    check (result_version > 0),
  constraint occupational_therapy_assessment_operations_state_check
    check (result_state in ('draft', 'signed', 'corrected')),
  constraint occupational_therapy_assessment_operations_reauth_check check (
...
comment on table private.occupational_therapy_assessment_operations is
  'Append-only actor-scoped exact-replay receipts for page 33 writes.';

create index occupational_therapy_assessments_scope_latest_idx
  on public.occupational_therapy_assessment_versions (
    organization_id, branch_id, client_id, assessed_on desc,
    created_at desc, assessment_key, version desc
  );
create index occupational_therapy_assessments_chain_idx
  on public.occupational_therapy_assessment_versions (
    organization_id, branch_id, assessment_key, version desc
  );
create index occupational_therapy_assessments_previous_idx
  on public.occupational_therapy_assessment_versions (previous_version_id)
  where previous_version_id is not null;
create index occupational_therapy_assessments_therapist_idx
  on public.occupational_therapy_assessment_versions (therapist_user_id);
create index occupational_therapy_assessments_signed_by_idx
  on public.occupational_therapy_assessment_versions (signed_by)
  where signed_by is not null;
create index occupational_therapy_assessments_reauth_idx
  on public.occupational_therapy_assessment_versions (signature_reauth_challenge_id)
  where signature_reauth_challenge_id is not null;
create index occupational_therapy_assessment_operations_result_idx
  on private.occupational_therapy_assessment_operations (
    organization_id, branch_id, client_id, result_assessment_key,
    result_version_id
  );
create index occupational_therapy_assessment_operations_reauth_idx
  on private.occupational_therapy_assessment_operations (reauth_challenge_id)
  where reauth_challenge_id is not null;

alter table public.occupational_therapy_assessment_versions enable row level security;
alter table public.occupational_therapy_assessment_versions force row level security;
alter table private.occupational_therapy_assessment_operations enable row level security;
alter table private.occupational_therapy_assessment_operations force row level security;

revoke all on table public.occupational_therapy_assessment_versions
  from public, anon, authenticated, service_role;
revoke all on table private.occupational_therapy_assessment_operations
  from public, anon, authenticated, service_role;

create or replace function private.occupational_therapy_history_is_append_only()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using errcode = '55000',
    message = 'occupational therapy assessment history is append-only';
end;
$$;

create or replace function private.occupational_therapy_current_authority(
  ...
  select auth.uid() is not null
    ...
    and exists (
      ...
      where profile.id = auth.uid()
        and (
          (p_permission = 'occupational_therapy_assessments.read'
...
create or replace function private.occupational_therapy_client_authority(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_permission text
...
set search_path = ''
as $$
  select private.occupational_therapy_current_authority(
      p_expected_organization_id, p_expected_branch_id, p_permission
    )
    and exists (
...
create or replace function private.require_occupational_therapy_reauth_evidence(
  p_actor uuid,
...
begin
  if p_actor is null or p_actor <> auth.uid()
    ...
    raise exception using errcode = '42501',
      message = 'current same-session recent AAL2 evidence is required for occupational_therapy signing';
  end if;
  begin
...
$$;
create or replace function private.mutate_occupational_therapy_assessment_guarded(
  p_action text,
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
declare
  ...
  v_now timestamptz;
  v_request_hash text;
  v_operation private.occupational_therapy_assessment_operations%rowtype;
  v_previous public.occupational_therapy_assessment_versions%rowtype;
  v_result public.occupational_therapy_assessment_versions%rowtype;
...
begin
  if p_action not in ('create_draft', 'revise_draft', 'sign', 'correct')
     ...
     or not private.occupational_therapy_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       case when p_action in ('sign', 'correct')
         then 'occupational_therapy_assessments.sign' else 'occupational_therapy_assessments.manage' end
     )
     or not private.occupational_therapy_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'occupational_therapy_assessments.read'
    ...
    raise exception using errcode = '42501',
      message = 'occupational_therapy assessment operation is not permitted';
  end if;
  ...
  if p_action = 'create_draft' then
    if p_assessment_key is not null or p_previous_version_id is not null
      ...
      raise exception using errcode = '22023',
        message = 'new occupational_therapy assessment chain input is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'occupational_therapy assessment chain input is invalid';
  end if;
  ...
  if p_action in ('create_draft', 'revise_draft', 'correct') then
    if p_assessed_on is null
       ...
       or not private.occupational_therapy_measurements_are_valid(p_measurements)
       ...
       or p_form_version_reference <> 'manual-occupational-therapy-v1'
      ...
      raise exception using errcode = '22023',
        message = 'manual occupational_therapy assessment content is invalid';
    end if;
    ...
    raise exception using errcode = '22023',
      message = 'occupational_therapy signing must use the exact current draft';
  end if;
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'occupational_therapy-operation:' || v_actor::text || ':' ||
      p_idempotency_key::text, 0
  ));
  select operation.* into v_operation
  from private.occupational_therapy_assessment_operations operation
  ...
  if v_operation.id is not null then
    if v_operation.request_hash <> v_request_hash then
      raise exception using errcode = '23505',
        message = 'occupational_therapy assessment idempotency conflict';
    end if;
    if not private.occupational_therapy_client_authority(
      ...
      raise exception using errcode = '42501',
        message = 'occupational_therapy assessment replay is not permitted';
    end if;
    select version_row.* into strict v_result
    from public.occupational_therapy_assessment_versions version_row
  ...
  perform pg_advisory_xact_lock(hashtextextended(
    'occupational_therapy-chain:' || p_expected_organization_id::text || ':' ||
  ...
  if p_action <> 'create_draft' then
    ...
    if v_previous.id is null or v_previous.id <> p_previous_version_id
      ...
      raise exception using errcode = '40001',
        message = 'occupational_therapy assessment version is stale';
    end if;
  end if;
  ...
  if p_action = 'create_draft' then
    ...
    if v_previous.record_state <> 'draft' then
      raise exception using errcode = '23514',
        message = 'signed occupational_therapy assessment requires a correction';
    end if;
    ...
    if v_previous.record_state <> 'draft' then
      raise exception using errcode = '23514',
        message = 'only the current occupational_therapy draft can be signed';
    end if;
    if v_previous.therapist_user_id <> v_actor then
      raise exception using errcode = '42501',
        message = 'only the current qualified occupational therapist may sign this draft';
    end if;
    ...
    if v_previous.record_state not in ('signed', 'corrected') then
      raise exception using errcode = '23514',
        message = 'only signed occupational_therapy content can be corrected';
    end if;
  ...
  if v_assessed_on > (v_now at time zone 'Asia/Taipei')::date then
    raise exception using errcode = '22023',
      message = 'future occupational_therapy assessment date is invalid';
  end if;
  if v_therapist_display_name is null or v_service_status is null then
    raise exception using errcode = '42501',
      message = 'occupational_therapy therapist identity or service status is invalid';
  end if;
  if p_action in ('sign', 'correct') then
    v_challenge_id := private.require_occupational_therapy_reauth_evidence(
      v_actor, v_now
    );
    ...
    if v_signer_display_name is null
      ...
      raise exception using errcode = '42501',
        message = 'occupational_therapy signer identity could not be verified';
    end if;
  end if;
  ...
  if not private.occupational_therapy_client_authority(
    ...
    raise exception using errcode = '42501',
      message = 'occupational_therapy assessment authority expired';
  end if;
  ...
  insert into public.occupational_therapy_assessment_versions (
    ...
    case when v_state = 'signed' then '人工職能治療評估簽署'
      when v_state = 'corrected' then '人工職能治療評估更正簽署'
      else null end,
    v_challenge_id, v_content_hash, v_now
  ) returning * into v_result;

  insert into private.occupational_therapy_assessment_operations (
  ...
  insert into public.audit_events (
    ...
    'occupational_therapy_assessment_versions', v_result.id::text,
    p_idempotency_key,
    array[
    ...
    jsonb_build_object(
      'workflow', 'page33_manual_occupational_therapy_v1',
  ...
  if not private.occupational_therapy_client_authority(
    ...
    select 1 from public.occupational_therapy_assessment_versions version_row
    ...
    raise exception using errcode = '42501',
      message = 'occupational_therapy assessment final verification failed';
  end if;
...
$$;
create or replace function public.create_occupational_therapy_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessed_on date,
...
as $$
  select * from private.mutate_occupational_therapy_assessment_guarded(
...
create or replace function public.revise_occupational_therapy_assessment_draft(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create or replace function public.sign_occupational_therapy_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create or replace function public.correct_occupational_therapy_assessment(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_assessment_key uuid,
...
create trigger occupational_therapy_assessment_versions_append_only
before update or delete on public.occupational_therapy_assessment_versions
for each row execute function private.occupational_therapy_history_is_append_only();
create trigger occupational_therapy_assessment_operations_append_only
before update or delete on private.occupational_therapy_assessment_operations
for each row execute function private.occupational_therapy_history_is_append_only();

create trigger occupational_therapy_assessment_versions_audit_row_change
after insert on public.occupational_therapy_assessment_versions
for each row execute function private.audit_row_change();
create trigger occupational_therapy_assessment_operations_audit_row_change
after insert on private.occupational_therapy_assessment_operations
for each row execute function private.audit_row_change();

create or replace function private.occupational_therapy_assessment_snapshot_bundle(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
...
as $$
  with terminal_visible as materialized (
    ...
    where version_row.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(
        version_row.client_id, 'occupational_therapy_assessments.read'
      )
      and not exists (
        select 1 from public.occupational_therapy_assessment_versions child
        where child.previous_version_id = version_row.id
    ...
    where client.organization_id = p_expected_organization_id
      ...
      and private.can_staff_access_client(
        client.id, 'occupational_therapy_assessments.read'
      )
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'version_history', case when item.assessment_key is null
        then '[]'::jsonb else coalesce((
          ...
          from (
            select history.*
            from public.occupational_therapy_assessment_versions history
...
create or replace function private.occupational_therapy_assessment_snapshot_response(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
  p_client_id uuid,
  p_therapist_user_id uuid,
...
begin
  if p_expected_organization_id is null or p_expected_branch_id is null
     ...
     or not private.occupational_therapy_current_authority(
       p_expected_organization_id, p_expected_branch_id,
       'occupational_therapy_assessments.read'
     ) then
    raise exception using errcode = '42501',
      message = 'occupational_therapy assessment snapshot is not permitted';
  end if;

  if p_client_id is not null and not private.occupational_therapy_client_authority(
       p_expected_organization_id, p_expected_branch_id, p_client_id,
       'occupational_therapy_assessments.read'
     ) then
    raise exception using errcode = '42501',
      message = 'occupational_therapy client filter is not permitted';
  end if;
  ...
  if p_therapist_user_id is not null and not exists (
    ...
    raise exception using errcode = '42501',
      message = 'occupational_therapy therapist filter is not permitted';
  end if;

  v_bundle := private.occupational_therapy_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
  ...
  insert into public.audit_events (
    ...
    'occupational_therapy_assessment_versions', null, '{}'::text[],
    jsonb_build_object(
      'projection', 'page33_manual_occupational_therapy_v1',
  ...
  v_after := private.occupational_therapy_assessment_snapshot_bundle(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_therapist_user_id, v_service_status, v_due_status, v_now
  );
  if not private.occupational_therapy_current_authority(
       p_expected_organization_id, p_expected_branch_id,
       'occupational_therapy_assessments.read'
     ) or v_after is distinct from v_bundle then
    raise exception using errcode = '42501',
      message = 'occupational_therapy assessment snapshot final verification failed';
  end if;
...
create or replace function public.occupational_therapy_assessment_snapshot(
  p_expected_organization_id uuid,
  p_expected_branch_id uuid,
...
set search_path = ''
as $$
  select * from private.occupational_therapy_assessment_snapshot_response(
    p_expected_organization_id, p_expected_branch_id, p_client_id,
    p_therapist_user_id, p_service_status, p_due_status
  );
$$;

create policy occupational_therapy_assessment_versions_staff_select
on public.occupational_therapy_assessment_versions for select to authenticated
using (
  private.can_staff_access_client(client_id, 'clients.read')
  and private.can_staff_access_client(client_id, 'occupational_therapy_assessments.read')
);

revoke all on function private.occupational_therapy_measurements_are_valid(jsonb)
  from public, anon, authenticated, service_role;
revoke all on function private.occupational_therapy_history_is_append_only()
  from public, anon, authenticated, service_role;
revoke all on function private.occupational_therapy_current_authority(uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.occupational_therapy_client_authority(uuid,uuid,uuid,text)
  from public, anon, authenticated, service_role;
revoke all on function private.require_occupational_therapy_reauth_evidence(uuid,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.mutate_occupational_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function private.occupational_therapy_assessment_snapshot_bundle(uuid,uuid,uuid,uuid,text,text,timestamptz)
  from public, anon, authenticated, service_role;
revoke all on function private.occupational_therapy_assessment_snapshot_response(uuid,uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

revoke all on function public.create_occupational_therapy_assessment_draft(uuid,uuid,uuid,date,date,text,jsonb,text,text,text,text,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.revise_occupational_therapy_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.sign_occupational_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.correct_occupational_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)
  from public, anon, authenticated, service_role;
revoke all on function public.occupational_therapy_assessment_snapshot(uuid,uuid,uuid,uuid,text,text)
  from public, anon, authenticated, service_role;

grant execute on function public.create_occupational_therapy_assessment_draft(uuid,uuid,uuid,date,date,text,jsonb,text,text,text,text,text,uuid)
  to authenticated;
grant execute on function public.revise_occupational_therapy_assessment_draft(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,uuid)
  to authenticated;
grant execute on function public.sign_occupational_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,uuid)
  to authenticated;
grant execute on function public.correct_occupational_therapy_assessment(uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)
  to authenticated;
grant execute on function public.occupational_therapy_assessment_snapshot(uuid,uuid,uuid,uuid,text,text)
  to authenticated;

-- SECURITY INVOKER wrappers need only the exact private entry point called.
grant execute on function private.mutate_occupational_therapy_assessment_guarded(text,uuid,uuid,uuid,uuid,uuid,integer,date,date,text,jsonb,text,text,text,text,text,text,uuid)
  to authenticated;
grant execute on function private.occupational_therapy_assessment_snapshot_response(uuid,uuid,uuid,uuid,text,text)
  to authenticated;

comment on function public.occupational_therapy_assessment_snapshot(uuid,uuid,uuid,uuid,text,text)
is 'Page 33 assigned-client list with latest manual, non-standardized assessment, occupational therapist, current service status, and staff-entered due date. No score or diagnosis.';
