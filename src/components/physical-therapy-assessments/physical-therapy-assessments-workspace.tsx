...
import type {
  PhysicalTherapyAssessmentFilters,
...
import {
  PhysicalTherapyAssessmentActions,
  PhysicalTherapyAssessmentFreshness,
} from "./physical-therapy-assessment-actions";
...
function assessmentStateText(item: PhysicalTherapyAssessmentListItem) {
...
function measurementStateText(item: PhysicalTherapyMeasurement) {
...
function ProfessionalNarratives({ item }: { item: PhysicalTherapyAssessmentListItem }) {
...
function AssessmentHistory({ item }: { item: PhysicalTherapyAssessmentListItem }) {
...
export function PhysicalTherapyAssessmentsWorkspace({
  canManage,
  canSign,
  filters,
  hasRecentAal2,
  ...
  snapshot: PhysicalTherapyAssessmentSnapshot | null;
}) {
  ...
  return <>
    ...
    <header className="page-heading core-care-heading">
      <div>
        ...
        <p className="page-heading__description">
          顯示最近人工評估、負責物理治療師及人工複評期限；每一版均可依當時內容完整重現。
        </p>
      </div>
    </header>
    ...
    <div className={`callout ${styles.manual}`} role="note">
      ...
      <span><strong>人工、非標準化紀錄：</strong>使用 <code>manual-physical-therapy-v1</code> 技術參照；測量項目由治療師人工命名並保存，不加總成分數，也不宣稱官方量表、公式或診斷。</span>
    </div>
    {snapshot.demo ? <div className={`callout ${styles.demo}`} role="status">
      ...
      <span>只載入目前機構、分支及指派個案。新增、修訂、簽署及更正者須為目前專業人員、具獨立物理治療權限，且每次寫入都需最近 15 分鐘 AAL2。</span>
    </div>}
    ...
    <section aria-label="物理治療評估摘要" className="metric-grid">
      {[
    ...
    <section className="panel">
      <div className="panel__header">
        <div className="panel__title">
          <h2>個案物理治療評估清單</h2>
          <p>{snapshot.matchingTotal} 位符合條件・快照 {formatTimestamp(snapshot.generatedAt)}・<PhysicalTherapyAssessmentFreshness demo={snapshot.demo} staleAfter={snapshot.staleAfter} /></p>
        </div>
      </div>
      ...
      {snapshot.items.length ? <>
        <div aria-label="物理治療評估清單，可左右捲動" className={`table-wrap ${styles.table}`} role="region" tabIndex={0}>
          <table className="data-table">
            ...
            <tbody>{snapshot.items.map((item) => <tr key={item.clientId}>
              ...
              <td><PhysicalTherapyAssessmentActions canManage={canManage} canSign={canSign} hasRecentAal2={hasRecentAal2} item={item} snapshot={snapshot} /></td>
            </tr>)}</tbody>
          </table>
        </div>
        <div className="mobile-records core-care-mobile">
          {snapshot.items.map((item) => <article className={`record-card ${styles.card}`} key={item.clientId}>
            ...
            <dl>
              ...
              <div><dt>物理治療師</dt><dd>{item.therapistDisplayName ?? "尚無"}</dd></div>
            ...
            <ProfessionalNarratives item={item} />
            <AssessmentHistory item={item} />
            <PhysicalTherapyAssessmentActions canManage={canManage} canSign={canSign} hasRecentAal2={hasRecentAal2} item={item} snapshot={snapshot} />
          </article>)}
        </div>
        ...
        <section className="empty-card core-care-state">
          ...
          <h2>沒有符合條件的指派個案</h2>
          <p>請調整個案、物理治療師、服務狀態或人工期限；系統不會擴大到其他分支或未指派個案。</p>
