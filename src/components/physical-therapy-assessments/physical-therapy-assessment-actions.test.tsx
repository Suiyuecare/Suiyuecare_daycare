...
import { buildDemoPhysicalTherapyAssessmentSnapshot } from "@/lib/physical-therapy-assessments/demo";
...
const demo = buildDemoPhysicalTherapyAssessmentSnapshot();
...
function renderUnassessed() {
  return render(<PhysicalTherapyAssessmentActions
    canManage
    canSign
    hasRecentAal2
    item={unassessed}
...
describe("physical therapy assessment client write boundary", () => {
  it("keeps every synthetic action visibly disabled", () => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    render(<PhysicalTherapyAssessmentActions
      canManage
      canSign
      hasRecentAal2
      item={demo.items[0]!}
  ...
  it("locks create to the exact row client and sends all manual fields", async () => {
    ...
    expect((fetchMock.mock.calls[0]![1] as RequestInit).headers)
      .toMatchObject({ "x-physical-therapy-operation": "create_draft" });
  ...
  it("disables signing until recent AAL2 exists", () => {
    ...
    expect(screen.getAllByRole("alert").every((alert) =>
      /最近 15 分鐘/u.test(alert.textContent ?? ""))).toBe(true);
  });

  it("disables draft creation until recent AAL2 exists", () => {
    render(<PhysicalTherapyAssessmentActions
      canManage
      canSign
      hasRecentAal2={false}
    ...
    />);
    fireEvent.click(screen.getByText("開始新評估草稿", { selector: "summary" }));
    expect(screen.getByRole("button", { name: "開始新評估草稿" }))
      .toHaveProperty("disabled", true);
    expect(screen.getByRole("alert").textContent).toMatch(/新增、修訂、簽署或更正/u);
  });

  it("announces stale data and resets for a newer snapshot", async () => {
    const { rerender } = render(<PhysicalTherapyAssessmentFreshness
      demo={false}
      staleAfter={new Date(Date.now() - 1_000).toISOString()}
    />);
    await screen.findByRole("status");
    rerender(<PhysicalTherapyAssessmentFreshness
      demo={false}
