...
export function DashboardWorkspace({ snapshot, serviceDate, loadError = false, canViewManagementDetails = false }: {
  ...
  return <>
    ...
    <TodayWorkList key={serviceDate} rows={buildTodayWorkRows(snapshot)} serviceDate={serviceDate} access={snapshot.sourceAccess} />
    <footer className="today-footer">
      ...
      {canViewManagementDetails && <details className="today-management"><summary>管理檢查明細</summary>
        <p>清單為目前授權可見的在案個案，尚未與當日排程比對；不是應服務分母或整體完成率。請假／未到不產生缺測或缺日誌待辦；既有草稿與待簽署仍需查閱。</p>
