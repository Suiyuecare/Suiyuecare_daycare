...
import type {
  GdsAnswer,
  GdsAssessmentFilters,
  GdsAssessmentListItem,
  GdsAssessmentSnapshot,
  GdsVersionHistoryItem,
...
function answerText(answer: GdsAnswer) {
  if (answer.state === "answered") return answer.value === "yes" ? "是" : "否";
  if (answer.state === "missing") return "缺值";
  return `不適用：${answer.reason}`;
}

function answerSummary(item: Pick<GdsAssessmentListItem, "answers">) {
...
function previewBandText(value: string | null) {
  return {
    reference_0_4: "候選 0–4 點區間",
    elevated_5_8: "候選 5–8 點區間",
    high_9_11: "候選 9–11 點區間",
    very_high_12_15: "候選 12–15 點區間",
  }[value ?? ""] ?? "尚無候選區間";
}
...
function PreviewSummary({ item }: { item: GdsAssessmentListItem }) {
  ...
  return <div className={styles.statuses}>
    ...
    <small>{previewBandText(item.previewBandKey)}；不是正式風險分類。</small>
  </div>;
}

function VersionDetails({ version }: { version: GdsVersionHistoryItem }) {
  return <li>
    <strong>v{version.assessmentVersion}・{formatDate(version.assessedOn)}</strong>
    <span>{version.authorDisplayName}・不可變候選草稿</span>
    <ul className={styles.answerList}>
      {Object.entries(version.answers).map(([id, answer], index) =>
        <li key={id}>
...
function AssessmentHistory({ item }: { item: GdsAssessmentListItem }) {
...
function GdsAssessmentFreshness({ demo, staleAfter }: {
  demo: boolean;
  staleAfter: string;
}) {
...
export function GdsAssessmentsWorkspace({
  ...
  return <>
    ...
    <header className="page-heading core-care-heading">
      <div>
        ...
        <p className="page-heading__description">
          保存十五個明確答案狀態與不可變候選規則快照；目前只允許受治理的草稿、新版修訂與可重現試算。
        </p>
      </div>
    </header>
    ...
    <div className={`callout ${styles.candidate}`} role="note">
      ...
      <span><strong>候選規則尚未生效：</strong><code>{snapshot.ruleVersionId}</code> 仍待雙人核准。點數與區間只供規則覆核，不是官方分數、正式風險或診斷。</span>
    </div>
    ...
    {snapshot.demo ? <div className={`callout ${styles.demo}`} role="status">
      ...
      <span>只載入目前機構、分支與指派個案；草稿寫入需 clients.read、gds_assessments.read/manage 與最近 15 分鐘 AAL2，且在讀取內容前驗證。</span>
    </div>}
    <div className={`callout ${styles.offline}`} role="status">
      <ClipboardCheck aria-hidden="true" />
      <span><strong>尚未設定：</strong>正式規則、簽署、正式分數／風險、照顧決策、附件、匯出、離線同步與通知均 fail closed。</span>
    </div>
    ...
    <section aria-label="GDS 候選草稿摘要" className="metric-grid">
      {[
    ...
    <section className="panel">
      <div className="panel__header"><div className="panel__title">
        <h2>個案 GDS 候選草稿清單</h2>
        <p>{snapshot.matchingTotal} 位符合條件・快照 {formatTimestamp(snapshot.generatedAt)}・<GdsAssessmentFreshness demo={snapshot.demo} staleAfter={snapshot.staleAfter} /></p>
      </div></div>
      <form className={`filter-bar ${styles.filters}`} method="get">
        ...
        <label className="field field--compact"><span>答案狀態</span>
          <select defaultValue={filters.answerState} name="answers">
            <option value="all">全部</option>
            <option value="all_answered">十五題皆已回答</option>
      ...
      {snapshot.items.length ? <>
        <div aria-label="GDS 候選草稿清單，可左右捲動" className={`table-wrap ${styles.table}`} role="region" tabIndex={0}>
