...
import {
  ...
  type PhysicalTherapyServiceActionExpectation,
...
import type {
  ...
  PhysicalTherapyServiceValueState,
...
function readValue(data: FormData, prefix: string): PhysicalTherapyServiceValue {
...
function ValueEditor({
  ...
  defaultValue: PhysicalTherapyServiceValue;
  label: string;
  name: string;
}) {
    ...
    <label>
      ...
      <select
        ...
        onChange={(event) => setState(
          event.currentTarget.value as PhysicalTherapyServiceValueState,
        )}
        value={state}
      >
...
function ActionForm({
  ...
  record: PhysicalTherapyServiceRecord | null;
  snapshot: PhysicalTherapyServiceSnapshot;
}) {
  ...
  async function submit(form: HTMLFormElement) {
    ...
    const expectation: PhysicalTherapyServiceActionExpectation = {
      action,
    ...
    const response = await fetchWithTimeout("/api/physical-therapy-services", {
      method: action === "create_draft" ? "POST" : "PATCH",
      headers: {
        "content-type": "application/json",
        "idempotency-key": key,
        "x-physical-therapy-service-operation": action,
      },
    ...
    if (!response.ok) {
      const parsed = parsePhysicalTherapyServiceActionError(payload);
...
export function PhysicalTherapyServiceCreateAction(props: {
  canManage: boolean;
  canSign: boolean;
  hasRecentAal2: boolean;
  snapshot: PhysicalTherapyServiceSnapshot;
...
export function PhysicalTherapyServiceRecordActions(props: {
  canManage: boolean;
  canSign: boolean;
  hasRecentAal2: boolean;
  record: PhysicalTherapyServiceRecord;
  snapshot: PhysicalTherapyServiceSnapshot;
}) {
  if (props.snapshot.demo) {
...
export function PhysicalTherapyServiceFreshness({
  demo,
  staleAfter,
}: {
  demo: boolean;
