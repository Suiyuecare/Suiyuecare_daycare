...
import { DEFAULT_ROLE_CATEGORIES, roleScopeLabel } from "@/lib/domain/roles";
import type {
  ProfileKind,
...
export function RoleGovernanceWorkspace({
  ...
  return (
    <>
      ...
      <section className={styles.categoryGuide} aria-labelledby="role-categories-heading">
        <h2 id="role-categories-heading">職務類別與管理範圍</h2>
        <p>11 種標準分類是設定參考，不代表帳號已開通或已取得權限；實際授權請核對下方現況矩陣與成員角色。</p>
        <p><strong>機構主任可兼任護理、社工：</strong>須分別申請並核准對應角色，且核對專業資格；不因主任職稱自動取得護理或社工權限。</p>
        <details id="role-category-reference">
          <summary>查看 11 種標準職務分類</summary>
          <dl className={styles.categoryList}>
            {DEFAULT_ROLE_CATEGORIES.map((role) => <div key={role.key}>
              <dt>{role.label}</dt>
              <dd><span className={styles.categoryScope}>{roleScopeLabel(role.key)}</span>{role.summary}</dd>
            </div>)}
          </dl>
        </details>
      </section>
      ...
      <section className="panel">
        ...
        <div aria-label="可捲動的角色與權限現況表格" role="region" tabIndex={0} className={`table-wrap ${styles.tableWrap}`}><table className={`data-table ${styles.roleTable}`}><thead><tr><th scope="col">角色</th><th scope="col">來源</th><th scope="col">權限</th><th scope="col">狀態</th></tr></thead><tbody>{snapshot.roles.map((role) => <tr key={role.id}><td><span className={styles.stack}><strong>{currentGovernanceRoleName(role)}</strong><small>{role.roleKey}</small></span></td><td>{role.system ? "系統模板（不可改）" : "機構自訂"}</td><td><span className={styles.chips}>{role.permissionKeys.length ? role.permissionKeys.map((permission) => <code key={permission}>{permission}</code>) : <small>未授予一般權限</small>}</span></td><td><span className={`status-pill ${role.active ? "status-pill--success" : ""}`}>{role.active ? "有效" : "已停用"}</span></td></tr>)}</tbody></table></div>
        <div className={styles.mobileCards}>
          {snapshot.roles.map((role) => (
            <article className={styles.mobileCard} key={`${role.id}-mobile`}>
              <div className={styles.cardTop}>
                <div><strong>{currentGovernanceRoleName(role)}</strong><p className={styles.cardCode}>{role.roleKey}</p></div>
      ...
      <section className="panel">
        ...
        <div aria-label="可捲動的成員目前角色表格" role="region" tabIndex={0} className={`table-wrap ${styles.tableWrap}`}><table className={`data-table ${styles.memberTable}`}><thead><tr><th scope="col">成員</th><th scope="col">身分類型</th><th scope="col">層級</th><th scope="col">目前角色</th><th scope="col">狀態</th></tr></thead><tbody>{snapshot.memberships.map((membership) => <tr key={membership.id}><td><strong>{membership.displayName}{membership.currentActor ? "（本人）" : ""}</strong></td><td>{profileKindLabels[membership.profileKind]}</td><td>{membership.branchId ? "目前分支" : "機構層級"}</td><td><span className={styles.chips}>{membership.roleIds.length ? membership.roleIds.map((roleId) => <span key={roleId}>{currentGovernanceRoleName(rolesById.get(roleId))}</span>) : <small>尚無角色</small>}</span></td><td><span className={`status-pill ${membership.status === "active" ? "status-pill--success" : "status-pill--warning"}`}>{membershipStatusLabels[membership.status]}</span></td></tr>)}</tbody></table></div>
        <div className={styles.mobileCards}>
          {snapshot.memberships.map((membership) => (
            <article className={styles.mobileCard} key={`${membership.id}-mobile`}>
              ...
              <div><p className={styles.cardLabel}>目前角色</p><span className={styles.chips}>{membership.roleIds.length ? membership.roleIds.map((roleId) => <span key={roleId}>{currentGovernanceRoleName(rolesById.get(roleId))}</span>) : <small>尚無角色</small>}</span></div>
            </article>
          ))}
        </div>
      </section>
