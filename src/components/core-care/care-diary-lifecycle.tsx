...
function CareDiaryClientLifecycle({ clientId, enabled, canSign, canRevise = canSign, demo }: LifecycleProps) {
  ...
  async function action(record: DiaryRecord, kind: DiaryAction, extra: Record<string, unknown> = {}) {
    ...
    try {
      const response = await fetchWithTimeout(`/api/records/${frozen.body.record.id}/actions`, { method: "POST", cache: "no-store", headers: { "Content-Type": "application/json", "Idempotency-Key": frozen.key }, body: frozen.body.body });
      if (!response.ok && await isDefiniteCareRejection(response)) attempt.failed(response.status);
