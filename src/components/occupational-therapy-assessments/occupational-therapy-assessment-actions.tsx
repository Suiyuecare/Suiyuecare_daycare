...
import {
  ...
  type OccupationalTherapyActionExpectation,
...
import type {
  ...
  OccupationalTherapyMeasurementState,
...
type MeasurementDraft = OccupationalTherapyMeasurement & { key: string };
...
function initialMeasurements(item: OccupationalTherapyAssessmentListItem) {
...
function readMeasurements(data: FormData): OccupationalTherapyMeasurement[] {
  const count = Number(data.get("measurementCount"));
  return Array.from({ length: count }, (_, index) => {
    const state = String(
      data.get(`measurement.${index}.state`),
    ) as OccupationalTherapyMeasurementState;
...
function MeasurementEditor({ item }: { item: OccupationalTherapyAssessmentListItem }) {
      ...
      >
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => {
              const state = event.currentTarget.value as OccupationalTherapyMeasurementState;
              update(index, {
                state,
...
function AssessmentActionForm({
  ...
  item: OccupationalTherapyAssessmentListItem;
  snapshot: OccupationalTherapyAssessmentSnapshot;
}) {
  ...
  async function submit(form: HTMLFormElement) {
    ...
    let expectation: OccupationalTherapyActionExpectation;
    if (action === "sign") {
      body = { action, ...common };
      expectation = {
        action,
    ...
    const response = await fetchWithTimeout("/api/occupational-therapy-assessments", {
    ...
    if (!response.ok) {
      const parsed = parseOccupationalTherapyActionError(payload);
      if (parsed) {
    ...
    parseOccupationalTherapyActionSuccess(payload, expectation, response.status);
    operationKey.current = null;
    uncertain.current = false;
  }
      ...
      <fieldset className={styles.actionGrid} disabled={pending}>
        ...
        {editable ? <>
          ...
          <p className={styles.formReference}>
            表單參照：<code>manual-occupational-therapy-v1</code>（人工、非標準化；不計分、不套公式、不診斷）
          </p>
        </> : null}
        ...
        {action === "sign" ? <p className={styles.full}>
          將以目前職能治療師身分簽署 v{item.assessmentVersion} 草稿；簽署後只能追加有理由且連回原版的更正版。
        </p> : null}
        <button
...
export function OccupationalTherapyAssessmentActions({
  canManage,
  canSign,
  hasRecentAal2,
  item,
...
export function OccupationalTherapyAssessmentFreshness({
  demo,
  staleAfter,
}: {
  demo: boolean;
