...
import {
  ...
  OccupationalTherapyServiceFreshness,
...
function stateText(record: OccupationalTherapyServiceRecord) {
...
function ValueSummary({ label, value }: {
  label: string;
  value: OccupationalTherapyServiceValue;
}) {
...
function ServiceNarratives({ record }: { record: OccupationalTherapyServiceRecord }) {
...
function AssessmentReference({ record }: { record: OccupationalTherapyServiceRecord }) {
...
function ServiceHistory({ record }: { record: OccupationalTherapyServiceRecord }) {
...
export function OccupationalTherapyServicesWorkspace({
  canManage,
  canSign,
  filters,
  hasRecentAal2,
  ...
  snapshot: OccupationalTherapyServiceSnapshot | null;
}) {
  ...
  return <>
    ...
    <header className="page-heading core-care-heading">
      <div>
        ...
        <p className="page-heading__description">
          保存發生時間、內容、個案反應、人工建議、職能治療師與不可覆寫版本鏈。
        </p>
      </div>
      <OccupationalTherapyServiceCreateAction
        canManage={canManage}
    ...
    <div className={`callout ${styles.manual}`} role="note">
      ...
      <span><strong>獨立服務領域：</strong>第 33 頁最近已簽／已更正評估只作為唯讀參照；本頁不共用其版本或寫入權限。</span>
    </div>
    ...
    <section aria-label="職能治療服務摘要" className="metric-grid">
      {[
    ...
    <section className="panel">
      <div className="panel__header">
        <div className="panel__title">
          <h2>職能治療服務時間軸</h2>
          <p>{snapshot.matchingTotal} 筆符合條件・快照 {formatTimestamp(snapshot.generatedAt)}・<OccupationalTherapyServiceFreshness demo={snapshot.demo} staleAfter={snapshot.staleAfter} /></p>
        </div>
      </div>
      <form className={`filter-bar ${styles.filters}`} method="get">
        ...
        <label className="field field--compact"><span>職能治療師</span><select defaultValue={filters.therapistUserId ?? ""} name="therapist"><option value="">全部治療師</option>{snapshot.therapistOptions.map((item) => <option key={item.userId} value={item.userId}>{item.displayName}</option>)}</select></label>
      ...
      {snapshot.records.length ? <>
        <div aria-label="職能治療服務紀錄，可左右捲動" className={`table-wrap ${styles.table}`} role="region" tabIndex={0}>
          <table className="data-table">
            <thead><tr>{["個案／狀態", "發生時間／治療師", "內容／反應／建議", "評估參照", "版本／操作"].map((heading) => <th key={heading} scope="col">{heading}</th>)}</tr></thead>
            <tbody>{snapshot.records.map((record) => <tr data-service-row={record.recordKey} key={record.recordKey}>
              ...
              <td><ServiceHistory record={record} /><OccupationalTherapyServiceRecordActions canManage={canManage} canSign={canSign} hasRecentAal2={hasRecentAal2} record={record} snapshot={snapshot} /></td>
            </tr>)}</tbody>
          </table>
        </div>
        <div className="mobile-records core-care-mobile">
          {snapshot.records.map((record) => <article className={`record-card ${styles.card}`} data-service-card={record.recordKey} key={record.recordKey}>
            ...
            <dl><div><dt>職能治療師</dt><dd>{record.therapistDisplayName}</dd></div><div><dt>版本</dt><dd>v{record.recordVersion}</dd></div></dl>
            <ServiceNarratives record={record} />
            <AssessmentReference record={record} />
            <ServiceHistory record={record} />
            <OccupationalTherapyServiceRecordActions canManage={canManage} canSign={canSign} hasRecentAal2={hasRecentAal2} record={record} snapshot={snapshot} />
          </article>)}
        </div>
