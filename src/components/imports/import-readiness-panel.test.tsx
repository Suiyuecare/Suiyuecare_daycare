...
describe("import provenance browser", () => {
  ...
  it("filters and resets pagination, counts come from the same snapshot", () => {
    ...
    fireEvent.change(screen.getByLabelText("來源父層路徑"), { target: { value: "A/一" } });
  ...
  it("links to the inventory without expanding page count or claiming promotion", () => {
    ...
    expect(screen.getByRole("link", { name: "前往資料盤點與缺漏追蹤" })).toHaveAttribute("href", "/app/staff/governance/integrations-audit#data-inventory");
  });
});
