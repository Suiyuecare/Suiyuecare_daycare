...
function serviceStatusText(value: ChewingAssessmentListItem["serviceStatus"]) {
  return {
    active: "服務中",
    suspended: "暫停",
    transferred: "已轉出",
...
function answerText(answer: ChewingAnswer) {
...
function answerSummary(item: Pick<ChewingAssessmentListItem, "answers">) {
...
function PreviewSummary({ item }: { item: ChewingAssessmentListItem }) {
  ...
  return <div className={styles.statuses}>
    ...
    <small>只重播逐欄狀態，不是正式分數或能力分級。</small>
    <small>未建立或通知任何營養或吞嚥轉介。</small>
  </div>;
}

function VersionDetails({ version }: { version: ChewingVersionHistoryItem }) {
  return <li>
    ...
    <ul className={styles.answerList}>
      {Object.entries(version.answers).map(([id, answer]) =>
        <li key={id}>
          <span>{CHEWING_OBSERVATION_LABELS[id as keyof typeof CHEWING_OBSERVATION_LABELS]}</span>
          <span>{answerText(answer)}</span>
        </li>)}
    </ul>
    <p><strong>人工觀察重播：</strong>{version.previewStatus === "incomplete"
      ? "不完整，不產生項目數"
      : `${version.previewObservedCount} 項已出現；非分數、無能力分級`}</p>
...
function AssessmentHistory({ item }: { item: ChewingAssessmentListItem }) {
...
function ChewingAssessmentFreshness({ demo, staleAfter }: {
  demo: boolean;
  staleAfter: string;
}) {
...
export function ChewingAssessmentsWorkspace({
  canManage,
  filters,
  hasRecentAal2,
  loadError = false,
  ...
  snapshot: ChewingAssessmentSnapshot | null;
}) {
  ...
  return <>
    ...
    <header className="page-heading core-care-heading">
      <div>
        ...
        <p className="page-heading__description">
          正式咀嚼評估工具尚未提供；本頁只保存六項中性人工咀嚼觀察的四種明確狀態、不可變候選欄位快照與新版修訂。
        </p>
      </div>
    </header>
    ...
    <div className={`callout ${styles.candidate}`} role="note">
      ...
      <span><strong>這不是正式咀嚼評估工具：</strong><code>{snapshot.ruleVersionId}</code> 只保存人工未標準化候選欄位。正式題目文字、授權來源、權重、分數與能力分級均未配置。</span>
    </div>
    <div className={`callout ${styles.blocked}`} role="status">
      <Ban aria-hidden="true" />
      <span><strong>正式用途已封鎖：</strong>正式工具與規則雙人發布前，API 與資料庫拒絕正式簽署、更正生效、分數、能力分級、診斷、照顧決策及營養／吞嚥轉介。</span>
    </div>
    {snapshot.demo ? <div className={`callout ${styles.demo}`} role="status">
      ...
      <span>只載入目前機構、分支與指派個案；只有具專頁權限的 professional 可寫入，並需 clients.read、chewing_assessments.read/manage 與最近 15 分鐘 AAL2，且在讀取內容前驗證。</span>
    </div>}
    <div className={`callout ${styles.offline}`} role="status">
      <ClipboardCheck aria-hidden="true" />
      <span><strong>尚未設定：</strong>正式工具、授權來源、權重、簽署／更正、分數／能力分級、診斷、照顧決策、營養／吞嚥轉介、附件、匯出、離線同步與通知均 fail closed。</span>
    </div>
    ...
    <section aria-label="人工咀嚼觀察草稿摘要" className="metric-grid">
      {[
    ...
    <section className="panel">
      <div className="panel__header"><div className="panel__title">
        <h2>個案人工咀嚼觀察草稿清單</h2>
        <p>{snapshot.matchingTotal} 位符合條件・快照 {formatTimestamp(snapshot.generatedAt)}・<ChewingAssessmentFreshness demo={snapshot.demo} staleAfter={snapshot.staleAfter} /></p>
      </div></div>
      ...
      {snapshot.items.length ? <>
        <div aria-label="人工咀嚼觀察草稿清單，可左右捲動" className={`table-wrap ${styles.table}`} role="region" tabIndex={0}>
          ...
          <tbody>{snapshot.items.map((item) => {
            ...
            return <tr key={item.clientId}>
              ...
              <td><ChewingAssessmentActions canManage={canManage} hasRecentAal2={hasRecentAal2} item={item} snapshot={snapshot} /></td>
            </tr>;
          })}</tbody></table>
        </div>
        <div className="mobile-records core-care-mobile">
          {snapshot.items.map((item) => {
            ...
            return <article className={`record-card ${styles.card}`} key={item.clientId}>
              ...
              <ChewingAssessmentActions canManage={canManage} hasRecentAal2={hasRecentAal2} item={item} snapshot={snapshot} />
            </article>;
          })}
        </div>
