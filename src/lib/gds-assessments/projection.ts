...
const historySchema = z.object({
  ...
  rule_snapshot: gdsRuleSnapshotSchema,
...
const itemSchema = z.object({
  ...
  answers: gdsAnswersSchema.nullable(),
  rule_version_id: z.literal(GDS_RULE_VERSION).nullable(),
  rule_snapshot: gdsRuleSnapshotSchema.nullable(),
  rule_snapshot_hash: sha256.nullable(),
  governance_status: z.literal("candidate_unactivated").nullable(),
  preview_status: z.enum(GDS_PREVIEW_STATUSES).nullable(),
  preview_candidate_points: nullablePreviewInteger,
  preview_band_key: text(80).nullable(),
...
export type GdsAssessmentSnapshotSourceRow = z.input<typeof sourceSchema>;

function invalid(): never {
  throw new Error("INVALID_GDS_ASSESSMENT_SNAPSHOT");
}

function previewMatches(value: z.output<typeof historySchema>) {
  const computed = buildGdsTrialPreview(value.answers);
...
export function projectGdsAssessmentSnapshot(input: {
  row: unknown;
  ...
  const items = row.items.map((item) => {
    ...
    if (item.version_id === null) {
      ...
      const preview = buildGdsTrialPreview(item.answers!);
      if (
...
export function filterDemoGdsAssessmentSnapshot(
