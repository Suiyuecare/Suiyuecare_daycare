...
import {
  ...
  parseOccupationalTherapyActionError,
  parseOccupationalTherapyActionSuccess,
  parseOccupationalTherapyAssessmentMutation,
  parseOccupationalTherapyOperationResult,
} from "./parser";
import { filterDemoOccupationalTherapyAssessmentSnapshot } from "./projection";
...
const createBody = {
  ...
  measurements,
  functionalObservation: "合成示例：在熟悉環境可依簡短提示完成桌面活動。",
  goals: "合成示例：逐步減少活動中的口頭提示。",
  recommendations: "合成示例：保留視覺提示與充分反應時間。",
  followUpPlan: "合成示例：下次人工複評比較提示需求。",
  formVersionReference: "manual-occupational-therapy-v1" as const,
};
...
function row(overrides: Record<string, unknown> = {}) {
  return {
    ...
    form_version_reference: "manual-occupational-therapy-v1",
...
describe("page 33 occupational therapy assessment contracts", () => {
  it("builds a synthetic manual-only snapshot with unavailable capabilities explicit", () => {
    const snapshot = buildDemoOccupationalTherapyAssessmentSnapshot();
  ...
  it("filters demo rows by exact client", () => {
    ...
    const filtered = filterDemoOccupationalTherapyAssessmentSnapshot(snapshot, {
  ...
  it("accepts exact manual measurements and actor-scoped idempotency", () => {
    expect(parseCreateOccupationalTherapyDraft(createBody, idempotencyKey)).toEqual({
      ...createBody,
      idempotencyKey,
    });
  });
  ...
  it("preserves a numeric measurement as exact text instead of calculating", () => {
    const parsed = parseCreateOccupationalTherapyDraft(createBody, idempotencyKey);
    expect(parsed.measurements[0]).toMatchObject({ value: "12.50", unit: "分鐘" });
  });
  ...
  it.each([
    ...
    expect(() => parseCreateOccupationalTherapyDraft({
      ...createBody,
  ...
  it("rejects reverse dates and a missing idempotency key", () => {
    ...
    expect(() => parseCreateOccupationalTherapyDraft(createBody, null))
      .toThrow(/冪等鍵/u);
  });

  it("keeps sign input strict and content-free", () => {
    expect(parseOccupationalTherapyAssessmentMutation({
      action: "sign",
      clientId,
      assessmentKey,
  ...
  it("parses only a complete database receipt", () => {
    expect(parseOccupationalTherapyOperationResult(row(), "create_draft"))
      .toMatchObject({
        action: "create_draft",
        clientId,
        therapistUserId,
    ...
    expect(() => parseOccupationalTherapyOperationResult({
      ...row(),
      clinical_score: 5,
  ...
  it("correlates HTTP status, replay, client and immutable chain", () => {
    expect(parseOccupationalTherapyActionSuccess(success(), {
      action: "create_draft", clientId,
    }, 201).data.clientId).toBe(clientId);
    expect(parseOccupationalTherapyActionSuccess(success({ replayed: true }), {
      action: "create_draft", clientId,
  ...
  it("parses only structured API errors", () => {
    expect(parseOccupationalTherapyActionError({
      requestId,
      status: "error",
      data: null,
      errors: [{ code: "OCCUPATIONAL_THERAPY_VERSION_CONFLICT", message: "請重新載入" }],
    })?.errors[0]?.code).toBe("OCCUPATIONAL_THERAPY_VERSION_CONFLICT");
    expect(parseOccupationalTherapyActionError({ error: "raw stack" })).toBeNull();
  });
});
