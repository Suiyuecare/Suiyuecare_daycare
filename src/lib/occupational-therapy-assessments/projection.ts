...
import { occupationalTherapyMeasurementsSchema } from "./parser";
import {
  CLIENT_SERVICE_STATUSES,
  OCCUPATIONAL_THERAPY_RECORD_STATES,
  type OccupationalTherapyAssessmentFilters,
  type OccupationalTherapyAssessmentSnapshot,
} from "./types";
...
const itemSchema = z.object({
  ...
  record_state: z.enum(OCCUPATIONAL_THERAPY_RECORD_STATES).nullable(),
  ...
  measurements: occupationalTherapyMeasurementsSchema.nullable(),
  ...
  form_version_reference: z.literal("manual-occupational-therapy-v1").nullable(),
  correction_reason: narrative(1000).nullable(),
  signed_at: timestamp.nullable(),
  signer_display_name: narrative(120).nullable(),
...
export type OccupationalTherapyAssessmentSnapshotSourceRow =
...
function invalid(): never {
  throw new Error("INVALID_OCCUPATIONAL_THERAPY_SNAPSHOT");
}
...
export function projectOccupationalTherapyAssessmentSnapshot(input: {
  row: unknown;
...
export function filterDemoOccupationalTherapyAssessmentSnapshot(
