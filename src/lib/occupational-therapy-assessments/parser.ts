...
import {
  ...
  OCCUPATIONAL_THERAPY_MEASUREMENT_STATES,
  OCCUPATIONAL_THERAPY_RECORD_STATES,
  type CreateOccupationalTherapyDraftInput,
  type OccupationalTherapyAssessmentMutationInput,
  type OccupationalTherapyAssessmentOperationResult,
} from "./types";
...
export const occupationalTherapyMeasurementSchema = z.object({
  name: narrative(120),
  state: z.enum(OCCUPATIONAL_THERAPY_MEASUREMENT_STATES),
...
export const occupationalTherapyMeasurementsSchema = z.array(
  occupationalTherapyMeasurementSchema,
...
const operationRowSchema = z.object({
  ...
  record_state: z.enum(OCCUPATIONAL_THERAPY_RECORD_STATES),
  assessed_on: date,
  therapist_user_id: uuid,
  service_status_at_assessment: z.enum(CLIENT_SERVICE_STATUSES),
  reassessment_due_on: date,
  form_version_reference: z.literal("manual-occupational-therapy-v1"),
...
const successSchema = z.object({
  ...
  data: z.object({
    ...
    recordState: z.enum(OCCUPATIONAL_THERAPY_RECORD_STATES),
    assessedOn: date,
...
export function parseCreateOccupationalTherapyDraft(
...
export function parseOccupationalTherapyAssessmentMutation(
  body: Record<string, unknown>,
  idempotencyKey: string | null,
  ...
  if (!schema) invalid("不支援的職能治療評估操作。", "action");
  ...
  return {
    ...data,
    idempotencyKey: parseIdempotencyKey(idempotencyKey),
  } as OccupationalTherapyAssessmentMutationInput;
}

export function parseOccupationalTherapyOperationResult(
  value: unknown,
  action: OccupationalTherapyAssessmentOperationResult["action"],
): Omit<OccupationalTherapyAssessmentOperationResult, "persisted" | "demo"> {
...
export type OccupationalTherapyActionExpectation = {
  action: OccupationalTherapyAssessmentOperationResult["action"];
  clientId: string;
...
export function parseOccupationalTherapyActionSuccess(
  value: unknown,
  expectation: OccupationalTherapyActionExpectation,
  httpStatus: number,
) {
  const parsed = successSchema.safeParse(value);
  if (!parsed.success) throw new Error("INVALID_OCCUPATIONAL_THERAPY_SUCCESS");
  ...
  ) throw new Error("MISMATCHED_OCCUPATIONAL_THERAPY_SUCCESS");
  return parsed.data;
}

export function parseOccupationalTherapyActionError(value: unknown) {
