...
import { isDailyWorkflowShift } from "./workflow-links";
...
export function parseDailyWorkSelection(query: Record<string, string | string[] | undefined>, now = new Date()) {
  ...
  const invalidShift = query.shift !== undefined && !isDailyWorkflowShift(query.shift);
  return {
    serviceDate,
    selectedClientId: typeof query.client === "string" && !invalidClient ? query.client.toLowerCase() : undefined,
    selectedShift: isDailyWorkflowShift(query.shift) ? query.shift : undefined,
    invalid: invalidDate || invalidClient || invalidShift,
  };
}
