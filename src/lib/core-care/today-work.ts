...
export function buildTodayWorkRows(snapshot: DailyCareSnapshot, roster?: CareRosterSnapshot): TodayWorkRow[] {
  ...
  return snapshot.clients.filter((client) => !planned || planned.some((row) => row.clientId === client.clientId)).map((client): TodayWorkRow => {
    ...
    if (access.attendance && !attendance && (planned || client.applicability?.attendance === "expected")) tasks.push("attendance");
    ...
    if (access.measurements && !offDay && (plannedMeasurements ? plannedMeasurements.some((task) => task.status === "pending") : client.applicability?.care === "expected" && !vital)) tasks.push("measurements");
    if (access.careDiaries && ((!offDay && (plannedDiaries ? plannedDiaries.some((task) => task.status === "pending") : client.applicability?.care === "expected" && !diary)) || (diary && ["draft", "submitted", "voided"].includes(diary.status)))) tasks.push("diary");
    const first = tasks[0];
