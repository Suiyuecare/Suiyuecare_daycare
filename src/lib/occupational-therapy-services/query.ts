...
import { OCCUPATIONAL_THERAPY_SERVICE_RECORD_STATES } from "./types";
...
export function parseOccupationalTherapyServiceQuery(
  raw: Record<string, string | string[] | undefined>,
) {
  for (const [key, value] of Object.entries(raw)) {
    if (!allowedKeys.has(key) || Array.isArray(value)) {
      throw new Error("INVALID_OCCUPATIONAL_THERAPY_SERVICE_QUERY");
    }
  }
  const parsed = z.object({
    ...
    state: z.enum(OCCUPATIONAL_THERAPY_SERVICE_RECORD_STATES).optional(),
    q: keyword.optional(),
