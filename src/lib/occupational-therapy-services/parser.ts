...
import {
  ...
  OCCUPATIONAL_THERAPY_SERVICE_VALUE_STATES,
  type CreateOccupationalTherapyServiceDraftInput,
  type OccupationalTherapyServiceMutationInput,
  type OccupationalTherapyServiceOperationResult,
} from "./types";
...
export const occupationalTherapyServiceValueSchema = z.object({
  state: z.enum(OCCUPATIONAL_THERAPY_SERVICE_VALUE_STATES),
...
const serviceFields = {
  ...
  serviceContent: occupationalTherapyServiceValueSchema,
  clientReaction: occupationalTherapyServiceValueSchema,
...
const successSchema = z.object({
  ...
  data: z.object({
    ...
    recordState: z.enum(OCCUPATIONAL_THERAPY_SERVICE_RECORD_STATES),
    occurredAt: timestamp,
...
export function parseCreateOccupationalTherapyServiceDraft(
...
export function parseOccupationalTherapyServiceMutation(
  body: Record<string, unknown>,
  idempotencyKey: string | null,
  ...
  if (!schema) invalid("不支援的職能治療服務紀錄操作。", "action");
  ...
  } as OccupationalTherapyServiceMutationInput;
}

export function parseOccupationalTherapyServiceOperationResult(
  value: unknown,
  action: OccupationalTherapyServiceOperationResult["action"],
): Omit<OccupationalTherapyServiceOperationResult, "persisted" | "demo"> {
...
export type OccupationalTherapyServiceActionExpectation = {
  action: OccupationalTherapyServiceOperationResult["action"];
...
export function parseOccupationalTherapyServiceActionSuccess(
  value: unknown,
  expectation: OccupationalTherapyServiceActionExpectation,
  httpStatus: number,
) {
  const parsed = successSchema.safeParse(value);
  if (!parsed.success) throw new Error("INVALID_OCCUPATIONAL_THERAPY_SERVICE_SUCCESS");
  ...
  ) throw new Error("MISMATCHED_OCCUPATIONAL_THERAPY_SERVICE_SUCCESS");
  return parsed.data;
}

export function parseOccupationalTherapyServiceActionError(value: unknown) {
