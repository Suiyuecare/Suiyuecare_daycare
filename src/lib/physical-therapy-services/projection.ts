...
import { physicalTherapyServiceValueSchema } from "./parser";
import {
  CLIENT_SERVICE_STATUSES,
  PHYSICAL_THERAPY_SERVICE_RECORD_STATES,
  type PhysicalTherapyServiceFilters,
  type PhysicalTherapyServiceRecord,
  type PhysicalTherapyServiceSnapshot,
} from "./types";
...
const versionHistorySchema = z.object({
  ...
  service_content: physicalTherapyServiceValueSchema,
  client_reaction: physicalTherapyServiceValueSchema,
...
export type PhysicalTherapyServiceSnapshotSourceRow =
  z.input<typeof sourceSchema>;

function invalid(): never {
  throw new Error("INVALID_PHYSICAL_THERAPY_SERVICE_PROJECTION");
}
...
export function projectPhysicalTherapyServiceSnapshot(input: {
  row: unknown;
...
export function filterDemoPhysicalTherapyServiceSnapshot(
