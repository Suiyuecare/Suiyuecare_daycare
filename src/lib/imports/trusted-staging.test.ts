...
import {
  stageTrustedHtmlImport, stageTrustedIntakeHtmlImport, trustedUploadOperationId,
...
describe("trusted HTML staging coordinator", () => {
  ...
  it("uses the dedicated Google intake reservation without fabricating AAL2", async () => {
    const test = harness();
    const ordinaryActor = { ...actor, assuranceLevel: "aal1" as const, recentAal2At: null };
    expect(await stageTrustedIntakeHtmlImport(test.dependencies, ordinaryActor, test.file, "google-intake-1")).toEqual(test.receipt);
    expect(test.userRpc).toHaveBeenCalledWith("reserve_intake_import_upload", expect.objectContaining({ p_expected_organization_id: actor.organizationId, p_expected_branch_id: actor.branchId }));
    expect(test.order).toEqual(["reserve", "archive", "complete"]);
    expect(ordinaryActor).toMatchObject({ assuranceLevel: "aal1", recentAal2At: null });
    await capturedError(stageTrustedHtmlImport(test.dependencies, ordinaryActor, test.file, "general-import-1"), "RECENT_AAL2_REQUIRED");
  });

  it("never archives or promotes a Google intake rejected by live database authority", async () => {
    const test = harness(); test.userRpc.mockResolvedValue({ data: null, error: { code: "42501", message: secret } });
    await expect(stageTrustedIntakeHtmlImport(test.dependencies, { ...actor, assuranceLevel: "aal1", recentAal2At: null }, test.file, "denied-intake")).rejects.toBeInstanceOf(ImportError);
    expect(test.archive).not.toHaveBeenCalled(); expect(test.workerRpc).not.toHaveBeenCalled();
  });

  it("rejects a routine intake larger than 4 MiB before parsing or reserving", async () => {
    const test = harness();
    await capturedError(stageTrustedIntakeHtmlImport(test.dependencies, { ...actor, assuranceLevel: "aal1", recentAal2At: null }, { ...test.file, bytes: new Uint8Array(4 * 1024 * 1024 + 1) }, "oversized-intake"), "IMPORT_STAGING_INVALID_FILE");
    expect(test.userRpc).not.toHaveBeenCalled(); expect(test.archive).not.toHaveBeenCalled(); expect(test.workerRpc).not.toHaveBeenCalled();
  });
