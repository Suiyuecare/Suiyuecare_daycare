...
async function stageHtmlImport(
  dependencies: TrustedStagingDependencies,
  actor: ImportActor,
  file: HtmlImportFile,
  idempotencyKey: string,
  mode: "legacy" | "routine-intake",
  ...
  if (mode === "legacy") assertRecentActor(context);
  else if (!["aal1", "aal2"].includes(context.assuranceLevel)) throw new ImportError("IMPORT_STAGING_DENIED", "請先使用已核准的 Google 帳號登入。", 403);
  let validated: ReturnType<typeof validateHtmlImportFile>;
  let parsed: ParsedHtmlImport;
  let serializedPayload: string;
  try {
    // Single-case web intake has the same 4 MiB cap as multipart and its SQL
    // reservation. The separate high-risk general importer retains its own cap.
    const byteLimit = mode === "routine-intake" ? 4 * 1024 * 1024 : MAX_HTML_IMPORT_BYTES;
    if (!(file.bytes instanceof Uint8Array) || file.bytes.byteLength > byteLimit) throw new Error();
  ...
  const rawReservation = await rpc(dependencies.userClient, mode === "legacy" ? "reserve_import_upload" : "reserve_intake_import_upload", {
...
/** Existing general import callers retain their original recent AAL2 gate. */
export async function stageTrustedHtmlImport(dependencies: TrustedStagingDependencies, actor: ImportActor, file: HtmlImportFile, idempotencyKey: string) {
  return stageHtmlImport(dependencies, actor, file, idempotencyKey, "legacy");
}
/** Only the dedicated single-file intake reservation RPC accepts approved
 * Google sessions. It independently validates the live actor before S3 writes. */
export async function stageTrustedIntakeHtmlImport(dependencies: TrustedStagingDependencies, actor: ImportActor, file: HtmlImportFile, idempotencyKey: string) {
  return stageHtmlImport(dependencies, actor, file, idempotencyKey, "routine-intake");
}
