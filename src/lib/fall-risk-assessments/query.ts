...
export function parseFallRiskAssessmentFilters(query: Query): {
  ...
  return {
    filters: {
      ...
      previewStatus: typeof preview === "string" && PREVIEW.has(preview)
        ? preview as FallRiskAssessmentFilters["previewStatus"] : "all",
      answerState: typeof answers === "string" && ANSWER_STATE.has(answers)
        ? answers as FallRiskAssessmentFilters["answerState"] : "all",
    },
