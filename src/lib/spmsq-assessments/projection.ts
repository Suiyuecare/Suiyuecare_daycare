...
import {
  ...
  spmsqAnswersSchema,
  spmsqCulturalContextSchema,
  spmsqEducationContextSchema,
  spmsqRuleSnapshotSchema,
} from "./parser";
import {
...
const historySchema = z.object({
  ...
  education_context: spmsqEducationContextSchema,
  cultural_context: spmsqCulturalContextSchema,
  rule_version_id: z.literal(SPMSQ_RULE_VERSION),
  rule_snapshot: spmsqRuleSnapshotSchema,
  rule_snapshot_hash: sha256,
  governance_status: z.literal("candidate_unactivated"),
  preview_status: z.enum(SPMSQ_PREVIEW_STATUSES),
  preview_raw_errors: nullablePreviewInteger,
  preview_adjusted_errors: nullablePreviewInteger,
...
const itemSchema = z.object({
  ...
  answers: spmsqAnswersSchema.nullable(),
  education_context: spmsqEducationContextSchema.nullable(),
  cultural_context: spmsqCulturalContextSchema.nullable(),
  rule_version_id: z.literal(SPMSQ_RULE_VERSION).nullable(),
  rule_snapshot: spmsqRuleSnapshotSchema.nullable(),
  rule_snapshot_hash: sha256.nullable(),
  governance_status: z.literal("candidate_unactivated").nullable(),
  preview_status: z.enum(SPMSQ_PREVIEW_STATUSES).nullable(),
  preview_raw_errors: nullablePreviewInteger,
  preview_adjusted_errors: nullablePreviewInteger,
  preview_band_key: text(80).nullable(),
...
const sourceSchema = z.object({
  ...
  cultural_adjustment_status: z.literal("not_configured_context_only"),
...
export type SpmsqAssessmentSnapshotSourceRow = z.input<typeof sourceSchema>;

function invalid(): never {
  throw new Error("INVALID_SPMSQ_ASSESSMENT_SNAPSHOT");
}

function previewMatches(value: z.output<typeof historySchema>) {
  const computed = buildSpmsqTrialPreview(
    value.answers,
    value.education_context,
  );
  return computed.status === value.preview_status &&
    computed.rawErrors === value.preview_raw_errors &&
    computed.adjustedErrors === value.preview_adjusted_errors &&
...
function terminalMatchesHistory(
  ...
  return history.version_id === item.version_id &&
    ...
    JSON.stringify(history.education_context) ===
      JSON.stringify(item.education_context) &&
    JSON.stringify(history.cultural_context) ===
      JSON.stringify(item.cultural_context) &&
    ...
    history.preview_raw_errors === item.preview_raw_errors &&
    history.preview_adjusted_errors === item.preview_adjusted_errors &&
...
function mapHistory(history: z.output<typeof historySchema>) {
  return {
    ...
    educationContext: history.education_context,
    culturalContext: history.cultural_context,
    ...
    previewRawErrors: history.preview_raw_errors,
    previewAdjustedErrors: history.preview_adjusted_errors,
    previewBandKey: history.preview_band_key,
    createdAt: history.created_at,
  };
}

export function projectSpmsqAssessmentSnapshot(input: {
  row: unknown;
  expectedOrganizationId: string;
  expectedBranchId: string;
  demo: boolean;
}): SpmsqAssessmentSnapshot {
  ...
  const items = row.items.map((item) => {
    const assessmentFields = [
      ...
      item.education_context, item.cultural_context, item.rule_version_id,
      item.rule_snapshot, item.rule_snapshot_hash, item.governance_status,
      item.preview_status, item.created_at,
    ];
    if (item.version_id === null) {
      if (
        assessmentFields.some((value) => value !== null) ||
        item.preview_raw_errors !== null ||
        item.preview_adjusted_errors !== null ||
      ...
      const preview = buildSpmsqTrialPreview(
        item.answers!,
        item.education_context!,
      );
      if (
        preview.status !== item.preview_status ||
        preview.rawErrors !== item.preview_raw_errors ||
        preview.adjustedErrors !== item.preview_adjusted_errors ||
    ...
    return {
      ...
      educationContext: item.education_context,
      ...
      previewRawErrors: item.preview_raw_errors,
      previewAdjustedErrors: item.preview_adjusted_errors,
  ...
  return {
    ...
    culturalAdjustmentStatus: row.cultural_adjustment_status,
...
export function filterDemoSpmsqAssessmentSnapshot(
  ...
  const items = snapshot.items.filter((item) =>
    ...
    (filters.educationState === "all" ||
      item.educationContext?.state === filters.educationState)
  );
  return {
    ...snapshot,
    items,
