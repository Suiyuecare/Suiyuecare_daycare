...
import {
  ...
  CHEWING_PREVIEW_STATUSES,
  CHEWING_RULE_VERSION,
  type CreateChewingDraftInput,
  type ChewingAnswers,
  type ChewingAssessmentMutationInput,
  type ChewingAssessmentOperationResult,
  type ChewingRuleSnapshot,
  type ChewingTrialPreview,
} from "./types";
...
export const chewingAnswerSchema = z.discriminatedUnion("state", [
  z.object({
...
const answerShape = Object.fromEntries(
  CHEWING_ITEM_IDS.map((id) => [id, chewingAnswerSchema]),
) as Record<(typeof CHEWING_ITEM_IDS)[number], typeof chewingAnswerSchema>;
export const chewingAnswersSchema = z.object(answerShape).strict();

const fieldDefinitionSchema = z.object({
  id: z.enum(CHEWING_ITEM_IDS),
  label: narrative(200),
  data_kind: z.literal("manual_presence_observation"),
}).strict();

export const chewingRuleSnapshotSchema = z.object({
  version_id: z.literal(CHEWING_RULE_VERSION),
  instrument: z.literal("manual_unstandardized_chewing_observations"),
  ...
  field_ids: z.array(z.enum(CHEWING_ITEM_IDS)).length(6),
  ...
  formal_tool_status: z.literal("not_configured"),
...
const fields = {
  clientId: uuid,
  assessedOn: date,
  answers: chewingAnswersSchema,
  ruleVersionId: z.literal(CHEWING_RULE_VERSION),
};
...
const operationRowSchema = z.object({
  ...
  rule_version_id: z.literal(CHEWING_RULE_VERSION),
  governance_status: z.literal("candidate_unactivated"),
  preview_status: z.enum(CHEWING_PREVIEW_STATUSES),
...
const successSchema = z.object({
  ...
  data: z.object({
    ...
    previewStatus: z.enum(CHEWING_PREVIEW_STATUSES),
...
function invalid(message: string, field?: string): never {
  throw new IntegrationError("INVALID_CHEWING_ASSESSMENT", message, 400, field);
}
...
export function buildChewingTrialPreview(answers: ChewingAnswers): ChewingTrialPreview {
...
export function parseCreateChewingDraft(
  body: Record<string, unknown>,
  idempotencyKey: string | null,
): CreateChewingDraftInput {
  const parsed = createSchema.safeParse(body);
  if (!parsed.success) invalid("個案、觀察日期或六項人工咀嚼觀察未通過驗證。");
  buildChewingTrialPreview(parsed.data.answers);
  return { ...parsed.data, idempotencyKey: parseIdempotencyKey(idempotencyKey) };
}

export function parseChewingAssessmentMutation(
  body: Record<string, unknown>,
  idempotencyKey: string | null,
): ChewingAssessmentMutationInput {
  const schema = body.action === "revise_draft" ? reviseSchema
    : body.action === "sign" ? signSchema
      : body.action === "correct" ? correctSchema : null;
  if (!schema) invalid("不支援的人工咀嚼觀察操作。", "action");
  ...
  }) as ChewingAssessmentMutationInput;
}

export function parseChewingOperationResult(
  value: unknown,
  action: ChewingAssessmentOperationResult["action"],
): Omit<ChewingAssessmentOperationResult, "persisted" | "demo"> {
...
export type ChewingActionExpectation = {
  action: ChewingAssessmentOperationResult["action"];
  clientId: string;
...
export function parseChewingActionSuccess(
  value: unknown,
  expectation: ChewingActionExpectation,
  httpStatus: number,
) {
  const parsed = successSchema.safeParse(value);
  if (!parsed.success) throw new Error("INVALID_CHEWING_SUCCESS");
  ...
  if (data.action !== expectation.action || data.clientId !== expectation.clientId ||
    ...
    throw new Error("MISMATCHED_CHEWING_SUCCESS");
  }
  return parsed.data;
}

export function parseChewingActionError(value: unknown) {
  const parsed = apiErrorSchema.safeParse(value);
  return parsed.success ? parsed.data : null;
}

export function parseChewingRuleSnapshot(value: unknown): ChewingRuleSnapshot {
  const parsed = chewingRuleSnapshotSchema.safeParse(value);
  if (!parsed.success) throw new Error("INVALID_CHEWING_RULE_SNAPSHOT");
  return parsed.data;
}
