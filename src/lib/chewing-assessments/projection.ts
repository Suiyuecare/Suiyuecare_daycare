...
const historySchema = z.object({
  ...
  rule_snapshot: chewingRuleSnapshotSchema,
...
const itemSchema = z.object({
  ...
  answers: chewingAnswersSchema.nullable(),
  rule_version_id: z.literal(CHEWING_RULE_VERSION).nullable(),
  rule_snapshot: chewingRuleSnapshotSchema.nullable(),
  rule_snapshot_hash: sha256.nullable(),
  governance_status: z.literal("candidate_unactivated").nullable(),
  preview_status: z.enum(CHEWING_PREVIEW_STATUSES).nullable(),
  preview_observed_count: nullablePreviewInteger,
...
const sourceSchema = z.object({
  ...
  formal_correction_status: z.literal("blocked_no_signed_record"),
  formal_score_status: z.literal("not_available"),
  formal_ability_classification_status: z.literal("not_available"),
  ...
  swallowing_referral_status: z.literal("not_configured"),
...
export type ChewingAssessmentSnapshotSourceRow = z.input<typeof sourceSchema>;

function invalid(): never {
  throw new Error("INVALID_CHEWING_ASSESSMENT_SNAPSHOT");
}

function previewMatches(value: z.output<typeof historySchema>) {
  const computed = buildChewingTrialPreview(value.answers);
...
export function projectChewingAssessmentSnapshot(input: {
  row: unknown;
  expectedOrganizationId: string;
  expectedBranchId: string;
  demo: boolean;
}): ChewingAssessmentSnapshot {
  ...
  const items = row.items.map((item) => {
    ...
    if (item.version_id === null) {
      ...
      const preview = buildChewingTrialPreview(item.answers!);
      if (
  ...
  return {
    ...
    formalAbilityClassificationStatus: row.formal_ability_classification_status,
    ...
    swallowingReferralStatus: row.swallowing_referral_status,
...
export function filterDemoChewingAssessmentSnapshot(
