...
export function parseNsiNutritionScreeningFilters(query: Query): {
  ...
  return {
    filters: {
      ...
      previewStatus: typeof preview === "string" && PREVIEW.has(preview)
        ? preview as NsiNutritionScreeningFilters["previewStatus"] : "all",
      answerState: typeof answers === "string" && ANSWER_STATE.has(answers)
        ? answers as NsiNutritionScreeningFilters["answerState"] : "all",
    },
    invalidFilters: !clientValid || !previewValid || !answersValid,
  };
}
