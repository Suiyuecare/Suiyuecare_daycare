...
describe("individually approved staff Google PKCE callback", () => {
  it("verifies flow-bound PKCE, user/claims and self-only allowlist, then admits the actual AAL1 Google session to the dashboard", async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledExactlyOnceWith("is_staff_login_allowed");
  ...
  it("activates only a prior server-owned approval after genuine OAuth and rechecks admission", async () => {
    ...
    expect(mocks.rpc.mock.calls).toEqual([
      ["is_staff_login_allowed"], ["activate_approved_staff_google_account"], ["is_staff_login_allowed"],
    ]);
    expect(mocks.signOut).not.toHaveBeenCalled();
  });

  it.each([false, null, "true", 1])("rejects activation result %j without granting from browser claims", async (data) => {
    mocks.rpc.mockResolvedValueOnce({ data: false, error: null }).mockResolvedValueOnce({ data, error: null });
    await expectDenied(await GET(request()));
    expect(mocks.rpc).toHaveBeenCalledTimes(2);
  });

  it("requires live admission after activation, including immediate revocation", async () => {
    mocks.rpc.mockResolvedValueOnce({ data: false, error: null })
      .mockResolvedValueOnce({ data: true, error: null }).mockResolvedValueOnce({ data: false, error: null });
    await expectDenied(await GET(request()));
  });

  it("does not fall back to activation when admission is unavailable", async () => {
    mocks.rpc.mockResolvedValueOnce({ data: false, error: { code: "PGRST202" } });
    await expectDenied(await GET(request()));
    expect(mocks.rpc).toHaveBeenCalledExactlyOnceWith("is_staff_login_allowed");
  });
