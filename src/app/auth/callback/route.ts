...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Existing staff use the pinned identity gate. A first sign-in can consume
    // only a prior owner approval; the database binds the actual Google subject
    // and fixed branch/role atomically. No browser-provided identity or role.
    let allowed = await supabase.rpc("is_staff_login_allowed");
    if (!allowed.error && allowed.data === false) {
      const activation = await supabase.rpc("activate_approved_staff_google_account");
      if (activation.error || activation.data !== true) throw new Error("GOOGLE_CALLBACK_DENIED");
      allowed = await supabase.rpc("is_staff_login_allowed");
    }
