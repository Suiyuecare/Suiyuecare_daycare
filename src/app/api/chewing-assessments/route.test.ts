...
import { chewingAnswersWithObservedCount } from "@/lib/chewing-assessments/demo";
...
const ruleVersionId = "chewing-manual-observations-candidate-v1";
const answers = chewingAnswersWithObservedCount(3);
...
const actor = {
  ...
  displayName: "測試評估人員",
  roles: ["professional"],
  scopes: ["clients.read", "chewing_assessments.read", "chewing_assessments.manage"],
  assuranceLevel: "aal2",
...
function request(method: "POST" | "PATCH", operation?: string) {
  return new Request("https://example.invalid/api/chewing-assessments", {
    method,
    headers: {
      "Content-Type": "application/json",
      "Idempotency-Key": idempotencyKey,
      ...(operation ? { "X-Chewing-Operation": operation } : {}),
    },
    body: "{}",
  });
}
...
describe("page 35 manual chewing observation API boundary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    [{ ...actor, scopes: ["clients.read", "chewing_assessments.read"] }, "CHEWING_NOT_AUTHORIZED"],
    [{ ...actor, scopes: ["chewing_assessments.read", "chewing_assessments.manage"] }, "CHEWING_NOT_AUTHORIZED"],
    [{ ...actor, roles: ["branch_supervisor"] }, "CHEWING_NOT_AUTHORIZED"],
  ...
  it("binds tenant, branch, exact client and only the manual candidate contract", async () => {
    stubs.maybeSingle.mockResolvedValue({ data: receipt(), error: null });
    const response = await POST(request("POST"));
    expect(response.status).toBe(201);
    expect(stubs.rpc).toHaveBeenCalledWith("create_chewing_assessment_draft", {
  ...
  it.each([
    ...
    [{ preview_observed_count: 2 }, "candidate preview"],
    ...
    expect((await response.json()).errors[0].code).toBe("CHEWING_RECEIPT_INVALID");
  });
  ...
  it("correlates a revision to the immutable previous version", async () => {
    ...
    const response = await PATCH(request("PATCH", "revise_draft"));
    expect(response.status).toBe(201);
    expect(stubs.rpc).toHaveBeenCalledWith("revise_chewing_assessment_draft",
  ...
  it("sign permission still reaches a fail-closed unactivated rule", async () => {
    stubs.authorizeStaffRequest.mockResolvedValue({
      ...actor,
      scopes: ["clients.read", "chewing_assessments.read", "chewing_assessments.sign"],
    });
    stubs.readJsonObject.mockResolvedValue({
      action: "sign",
      clientId,
    ...
    expect(payload.errors[0].code).toBe("CHEWING_RULE_NOT_ACTIVATED");
    expect(payload.data).toBeNull();
    expect(stubs.rpc).toHaveBeenCalledWith("sign_chewing_assessment",
      expect.objectContaining({ p_assessment_key: assessmentKey }));
  });

  it("correction requires sign scope and remains fail closed", async () => {
    stubs.authorizeStaffRequest.mockResolvedValue({
      ...actor,
      scopes: ["clients.read", "chewing_assessments.read", "chewing_assessments.sign"],
    });
    ...
    expect(stubs.rpc).toHaveBeenCalledWith("correct_chewing_assessment", {
      ...
      p_previous_version_id: versionId,
      p_expected_version: 1,
      p_correction_reason: "合成測試：請求正式更正。",
      p_idempotency_key: idempotencyKey,
    });
  });
  ...
  it.each([
    ["40001", 409, "CHEWING_VERSION_CONFLICT"],
    ["23505", 409, "CHEWING_IDEMPOTENCY_CONFLICT"],
    ["42501", 403, "CHEWING_NOT_AUTHORIZED"],
    ["55000", 409, "CHEWING_RULE_NOT_ACTIVATED"],
    ["22023", 400, "INVALID_CHEWING_ASSESSMENT"],
    ...
    expect(JSON.stringify(payload)).not.toMatch(
      /chewing_observation_01|stack|present|absent/u,
    );
  });
});
