...
import { nutritionAnswersWithObservedCount } from "@/lib/nsi-nutrition-screenings/demo";
...
const ruleVersionId = "nsi-manual-nutrition-observations-candidate-v1";
const answers = nutritionAnswersWithObservedCount(3);
...
const actor = {
  ...
  displayName: "測試評估人員",
  roles: ["professional"],
  scopes: ["clients.read", "nsi_nutrition_screenings.read", "nsi_nutrition_screenings.manage"],
  assuranceLevel: "aal2",
...
function request(method: "POST" | "PATCH", operation?: string) {
  return new Request("https://example.invalid/api/nsi-nutrition-screenings", {
    method,
    headers: {
      "Content-Type": "application/json",
      "Idempotency-Key": idempotencyKey,
      ...(operation ? { "X-Nsi-Nutrition-Operation": operation } : {}),
    },
    body: "{}",
  });
}
...
describe("page 14 manual nutrition observation API boundary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    [{ ...actor, scopes: ["clients.read", "nsi_nutrition_screenings.read"] }, "NSI_NUTRITION_NOT_AUTHORIZED"],
    [{ ...actor, scopes: ["nsi_nutrition_screenings.read", "nsi_nutrition_screenings.manage"] }, "NSI_NUTRITION_NOT_AUTHORIZED"],
  ...
  it("binds tenant, branch, exact client and only the manual candidate contract", async () => {
    ...
    expect(stubs.rpc).toHaveBeenCalledWith("create_nsi_nutrition_screening_draft", {
  ...
  it.each([
    ...
    expect((await response.json()).errors[0].code).toBe("NSI_NUTRITION_RECEIPT_INVALID");
  });
  ...
  it("correlates a revision to the immutable previous version", async () => {
    ...
    const response = await PATCH(request("PATCH", "revise_draft"));
    expect(response.status).toBe(201);
    expect(stubs.rpc).toHaveBeenCalledWith("revise_nsi_nutrition_screening_draft",
  ...
  it("sign permission still reaches a fail-closed unactivated rule", async () => {
    stubs.authorizeStaffRequest.mockResolvedValue({
      ...actor,
      scopes: ["clients.read", "nsi_nutrition_screenings.read", "nsi_nutrition_screenings.sign"],
    });
    stubs.readJsonObject.mockResolvedValue({
      action: "sign",
      clientId,
    ...
    expect(payload.errors[0].code).toBe("NSI_NUTRITION_RULE_NOT_ACTIVATED");
    expect(payload.data).toBeNull();
    expect(stubs.rpc).toHaveBeenCalledWith("sign_nsi_nutrition_screening",
  ...
  it.each([
    ["40001", 409, "NSI_NUTRITION_VERSION_CONFLICT"],
    ["23505", 409, "NSI_NUTRITION_IDEMPOTENCY_CONFLICT"],
    ["42501", 403, "NSI_NUTRITION_NOT_AUTHORIZED"],
    ["55000", 409, "NSI_NUTRITION_RULE_NOT_ACTIVATED"],
    ["22023", 400, "INVALID_NSI_NUTRITION_SCREENING"],
    ...
    expect(JSON.stringify(payload)).not.toMatch(
      /nutrition_observation_01|stack|present|absent/u,
    );
  });
});
