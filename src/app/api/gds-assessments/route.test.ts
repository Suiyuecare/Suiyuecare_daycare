...
import { candidateAnswers } from "@/lib/gds-assessments/demo";
...
const ruleVersionId = "gds-15-strict-complete-v1";
...
const actor = {
  ...
  displayName: "測試評估人員",
  roles: ["professional"],
  scopes: ["clients.read", "gds_assessments.read", "gds_assessments.manage"],
  assuranceLevel: "aal2",
...
function request(method: "POST" | "PATCH", operation?: string) {
  return new Request("https://example.invalid/api/gds-assessments", {
    method,
    headers: {
      "Content-Type": "application/json",
      "Idempotency-Key": idempotencyKey,
      ...(operation ? { "X-GDS-Operation": operation } : {}),
    },
    body: "{}",
  });
}
...
describe("GDS assessment API boundary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    [{ ...actor, scopes: ["clients.read", "gds_assessments.read"] }, "GDS_NOT_AUTHORIZED"],
    [{ ...actor, scopes: ["gds_assessments.read", "gds_assessments.manage"] }, "GDS_NOT_AUTHORIZED"],
  ...
  it("binds tenant, branch, exact client and only the GDS contract", async () => {
    stubs.maybeSingle.mockResolvedValue({ data: receipt(), error: null });
    const response = await POST(request("POST"));
    expect(response.status).toBe(201);
    expect(stubs.rpc).toHaveBeenCalledWith("create_gds_assessment_draft", {
  ...
  it.each([
    ...
    expect((await response.json()).errors[0].code).toBe("GDS_RECEIPT_INVALID");
  });
  ...
  it("correlates a revision to the immutable previous version", async () => {
    ...
    const response = await PATCH(request("PATCH", "revise_draft"));
    expect(response.status).toBe(201);
    expect(stubs.rpc).toHaveBeenCalledWith("revise_gds_assessment_draft",
  ...
  it("sign permission still reaches a fail-closed unactivated rule", async () => {
    stubs.authorizeStaffRequest.mockResolvedValue({
      ...actor,
      scopes: ["clients.read", "gds_assessments.read", "gds_assessments.sign"],
    });
    stubs.readJsonObject.mockResolvedValue({
      action: "sign",
      clientId,
    ...
    expect(payload.errors[0].code).toBe("GDS_RULE_NOT_ACTIVATED");
    expect(payload.data).toBeNull();
    expect(stubs.rpc).toHaveBeenCalledWith("sign_gds_assessment",
  ...
  it.each([
    ["40001", 409, "GDS_VERSION_CONFLICT"],
    ["23505", 409, "GDS_IDEMPOTENCY_CONFLICT"],
    ["42501", 403, "GDS_NOT_AUTHORIZED"],
    ["55000", 409, "GDS_RULE_NOT_ACTIVATED"],
    ["22023", 400, "INVALID_GDS_ASSESSMENT"],
    ...
    expect(JSON.stringify(payload)).not.toMatch(/gds_01|stack|yes|no/u);
  });
});
