...
function request(method: "POST" | "PATCH") {
  return new Request("https://example.invalid/api/occupational-therapy-assessments", {
    method,
    headers: {
...
describe("occupational therapy assessment API boundary", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it.each([
    ...
    [{ ...actor, scopes: ["clients.read", "occupational_therapy_assessments.read"] },
      "OCCUPATIONAL_THERAPY_NOT_AUTHORIZED"],
  ...
  it("rejects a denied mutation before parsing assessment content", async () => {
    stubs.authorizeStaffRequest.mockResolvedValue({
      ...actor,
      scopes: ["clients.read", "occupational_therapy_assessments.read"],
    });
    const response = await PATCH(request("PATCH"));
    expect(response.status).toBe(403);
    expect((await response.json()).errors[0].code)
      .toBe("OCCUPATIONAL_THERAPY_NOT_AUTHORIZED");
    expect(stubs.readJsonObject).not.toHaveBeenCalled();
    expect(stubs.rpc).not.toHaveBeenCalled();
  });
  ...
  it("binds tenant, branch, exact client and every structured manual field", async () => {
    ...
    expect(stubs.rpc).toHaveBeenCalledWith(
      "create_occupational_therapy_assessment_draft",
      {
        ...
        p_form_version_reference: "manual-occupational-therapy-v1",
  ...
  it.each([
    ...
    expect((await response.json()).errors[0].code)
      .toBe("OCCUPATIONAL_THERAPY_RECEIPT_INVALID");
  });
  ...
  it("correlates signing to the expected immutable chain", async () => {
    ...
    expect(stubs.requireRecentAal2).toHaveBeenCalledWith(actor);
    expect(stubs.rpc).toHaveBeenCalledWith(
      "sign_occupational_therapy_assessment",
  ...
  it.each([
    ["40001", 409, "OCCUPATIONAL_THERAPY_VERSION_CONFLICT"],
    ["23505", 409, "OCCUPATIONAL_THERAPY_IDEMPOTENCY_CONFLICT"],
    ["42501", 403, "OCCUPATIONAL_THERAPY_NOT_AUTHORIZED"],
    ["22023", 400, "INVALID_OCCUPATIONAL_THERAPY_ASSESSMENT"],
  ...
  it("rejects unsupported or invented clinical actions before a database call", async () => {
    ...
    expect((await response.json()).errors[0].code)
      .toBe("INVALID_OCCUPATIONAL_THERAPY_ASSESSMENT");
    expect(stubs.rpc).not.toHaveBeenCalled();
  });
});
