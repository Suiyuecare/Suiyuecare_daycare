import { ok } from "@/lib/api/response";
import { IntegrationError } from "@/lib/integrations/errors";
import {
  authorizeStaffRequest,
  databaseFailure,
  handleIntegrationRoute,
  readJsonObject,
} from "@/lib/integrations/http";
import { deterministicUuid } from "@/lib/integrations/security";
import {
  parseStaffTrainingRecordInput,
  parseStaffTrainingRecordReceipt,
  STAFF_TRAINING_RECORD_MAX_BYTES,
} from "@/lib/staff-training/parser";
import { createServerSupabaseClient } from "@/lib/supabase/server";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

function saveFailure(code: string | undefined) {
  if (code === "42501") return databaseFailure(
    "STAFF_TRAINING_NOT_AUTHORIZED",
    "目前角色、分支、員工範圍或在職狀態不允許保存此紀錄。",
    403,
  );
  if (code === "23505") return databaseFailure(
    "STAFF_TRAINING_IDEMPOTENCY_CONFLICT",
    "相同操作鍵已用於不同的教育訓練內容。",
    409,
  );
  if (["23514", "40001"].includes(code ?? "")) return databaseFailure(
    "STAFF_TRAINING_VERSION_CONFLICT",
    "訓練紀錄版本或人員狀態已變更；請重新載入。",
    409,
  );
  if (["22023", "22P02"].includes(code ?? "")) return databaseFailure(
    "INVALID_STAFF_TRAINING_RECORD",
    "課程、人員、時數、積分或更正資料未通過驗證。",
    400,
  );
  return databaseFailure(
    "STAFF_TRAINING_SAVE_UNCERTAIN",
    "訓練紀錄未確認完成；請保留相同操作鍵重試。",
    409,
  );
}

export async function POST(request: Request) {
  return handleIntegrationRoute(async (requestId) => {
    // Authorization is intentionally completed before reading the sensitive body.
    const actor = await authorizeStaffRequest();
    if (actor.demo) throw new IntegrationError(
      "DEMO_READ_ONLY",
      "展示模式只能檢視合成教育訓練資料，不會保存紀錄。",
      403,
    );
    if (!actor.scopes.includes("staff_training.read") ||
      !actor.scopes.includes("staff_training.manage")) throw new IntegrationError(
      "STAFF_TRAINING_NOT_AUTHORIZED",
      "目前角色沒有教育訓練紀錄管理權限。",
      403,
    );

    const input = parseStaffTrainingRecordInput(
      await readJsonObject(request, STAFF_TRAINING_RECORD_MAX_BYTES),
      request.headers.get("idempotency-key"),
    );
    const supabase = await createServerSupabaseClient();
    if (!supabase) throw databaseFailure(
      "SERVICE_NOT_CONFIGURED",
      "正式教育訓練資料服務尚未設定。",
      503,
    );
    const { data, error } = await supabase.rpc("append_staff_training_record", {
      p_expected_organization_id: actor.organizationId,
      p_expected_branch_id: actor.branchId,
      p_action: input.action,
      p_training_key: input.trainingKey,
      p_previous_version_id: input.previousVersionId,
      p_expected_base_version: input.expectedBaseVersion,
      p_staff_membership_id: input.staffMembershipId,
      p_course_title: input.courseTitle,
      p_training_date: input.trainingDate,
      p_starts_at: input.startsAt,
      p_ends_at: input.endsAt,
      p_course_type: input.courseType,
      p_hours: input.hours,
      p_credits: input.credits,
      p_provider_name: input.providerName,
      p_evidence_status: input.evidenceStatus,
      p_attachment_reference: input.attachmentReference,
      p_attachment_sha256: input.attachmentSha256,
      p_correction_reason: input.correctionReason,
      p_idempotency_key: deterministicUuid(
        "page71-staff-training-record",
        actor.organizationId,
        actor.userId,
        input.idempotencyKey,
      ),
    }).maybeSingle();
    if (error || !data) throw saveFailure(error?.code);
    const receipt = parseStaffTrainingRecordReceipt(
      data,
      input,
      actor.organizationId,
      actor.branchId,
    );
    return ok(
      { receipt, persisted: true, demo: false },
      receipt.replayed ? 200 : 201,
      requestId,
    );
  });
}
