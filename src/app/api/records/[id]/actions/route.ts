...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  return handleIntegrationRoute(async (requestId) => {
    const actor = await authorizeStaffRequest({ routinePermission: "care_records.write" });
    ...
    if (!actor.demo && actor.assuranceLevel !== "aal2" && !["edit", "submit"].includes(action.action)) {
      throw new IntegrationError("AAL2_REQUIRED", "簽署、更正或重開正式日誌須完成額外身分確認；原紀錄未被修改。", 403);
    }
