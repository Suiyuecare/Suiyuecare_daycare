...
import type {
  CreateOccupationalTherapyServiceDraftInput,
  OccupationalTherapyServiceMutationInput,
  OccupationalTherapyServiceOperationResult,
...
type OccupationalTherapyServiceOperation =
  "create_draft" | "revise_draft" | "sign" | "correct";

function requireOperationHeader(
  request: Request,
  allowed: readonly OccupationalTherapyServiceOperation[],
) {
  const operation = request.headers.get("x-occupational-therapy-service-operation");
  if (!allowed.includes(operation as OccupationalTherapyServiceOperation)) {
    throw new IntegrationError(
      "INVALID_OCCUPATIONAL_THERAPY_SERVICE",
      "缺少或不支援的職能治療服務紀錄操作標頭。",
      400,
      "x-occupational-therapy-service-operation",
    );
  }
  return operation as OccupationalTherapyServiceOperation;
}

function occupationalTherapyServiceFailure(errorCode?: string) {
  if (errorCode === "42501") {
    return databaseFailure(
      "OCCUPATIONAL_THERAPY_SERVICE_NOT_AUTHORIZED",
  ...
  if (errorCode === "40001") {
    return databaseFailure(
      "OCCUPATIONAL_THERAPY_SERVICE_VERSION_CONFLICT",
      "紀錄已有新版本；請重新載入後再操作。",
      409,
    );
  }
  if (errorCode === "23505") {
    return databaseFailure(
      "OCCUPATIONAL_THERAPY_SERVICE_IDEMPOTENCY_CONFLICT",
  ...
  if (["23514", "23503", "55000"].includes(errorCode ?? "")) {
    return databaseFailure(
      "OCCUPATIONAL_THERAPY_SERVICE_STATE_CONFLICT",
  ...
  return databaseFailure(
    "OCCUPATIONAL_THERAPY_SERVICE_SAVE_FAILED",
    "系統尚未確認操作結果；請保留內容並使用相同冪等鍵重試。",
  );
}

async function authorize(
  permission: "occupational_therapy_services.manage" |
  ...
  if (
    ...
    !actor.scopes.includes("occupational_therapy_services.read") ||
...
function correlateReceipt(
  result: Omit<OccupationalTherapyServiceOperationResult, "persisted" | "demo">,
  input: CreateOccupationalTherapyServiceDraftInput |
    OccupationalTherapyServiceMutationInput,
  actor: TenantContext,
) {
  ...
  if (
    ...
    throw new IntegrationError(
      "OCCUPATIONAL_THERAPY_SERVICE_RECEIPT_INVALID",
...
async function executeCreate(
  ...
  if (error || !data) throw occupationalTherapyServiceFailure(error?.code);
  return correlateReceipt(
    parseOccupationalTherapyServiceOperationResult(data, input.action),
    input,
    actor,
  );
}
...
async function executeMutation(
  input: OccupationalTherapyServiceMutationInput,
  actor: TenantContext,
) {
  const supabase = await createServerSupabaseClient();
  if (!supabase) {
  ...
  if (input.action === "revise_draft") {
    result = await supabase.rpc("revise_occupational_therapy_service_draft", {
      p_expected_organization_id: actor.organizationId,
      p_expected_branch_id: actor.branchId,
      p_client_id: input.clientId,
    ...
    result = await supabase.rpc("sign_occupational_therapy_service_record", {
      p_expected_organization_id: actor.organizationId,
      p_expected_branch_id: actor.branchId,
      p_client_id: input.clientId,
      p_record_key: input.recordKey,
    ...
    result = await supabase.rpc("correct_occupational_therapy_service_record", {
      p_expected_organization_id: actor.organizationId,
      p_expected_branch_id: actor.branchId,
      p_client_id: input.clientId,
      p_record_key: input.recordKey,
  ...
  if (result.error || !result.data) {
    throw occupationalTherapyServiceFailure(result.error?.code);
  }
  return correlateReceipt(
    parseOccupationalTherapyServiceOperationResult(result.data, input.action),
    input,
    actor,
  );
}
...
export async function POST(request: Request) {
  return handleIntegrationRoute(async (requestId) => {
    ...
    const actor = await authorize("occupational_therapy_services.manage", false);
    const input = parseCreateOccupationalTherapyServiceDraft(
...
export async function PATCH(request: Request) {
  return handleIntegrationRoute(async (requestId) => {
    ...
    const input = parseOccupationalTherapyServiceMutation(
      body,
      request.headers.get("idempotency-key"),
    );
